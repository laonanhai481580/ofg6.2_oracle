			...
			for(MfgPatrolDetailData data:datas){
				...
				for(int i=1;i<=80;i++){
					...
					if(value != null&&inspectionDate!=null){
						...
						tempMap.put(inspectionDate.toString()+"result" + i, value);
						if(i>tempMax){
							tempMax=i;
						}
					}
				...
				for(int i=1; i<=tempMax; i++){
					if(!tempMap.containsKey(inspectionDate==null?"":inspectionDate.toString()+"result" + i)){
						tempMap.put(inspectionDate==null?"":inspectionDate.toString()+"result" + i,null);
					}
				}
 ...
 <table class="form-table-border-left" style="border:0px;table-layout:fixed;">
	<thead>
		<tr id="checkItemsHeader" itemTr=true>
			...
			<td style="width:270px;padding:0px;border-top:0px;text-align: center;" tdFlag="<%=i %>"  records="true" >
				<input name="a_inspectionDate_<%=date%>" isDate=true type="text"  style="width:150px;"  value="<%=date%>" onchange="inspectionDateChange(this);"/>
				<a class="small-button-bg" style="float:right;" onclick="deleteRecord(this)" href="#" title="删除检验记录"><span class="ui-icon ui-icon-minus" style='cursor:pointer;'></span></a>
			</td>
			<% i++;}} %>
		</tr>
	...
	<tbody>
		<%
			...
			for(MfgPatrolDetail checkItem : checkItems){
				System.out.println(checkItem.getMfgPatrolDetailDatas().size());
				flag++;
		...
		%>
				...
				<%					
					...
					MfgPatrolDetailData data=null;
						for(MfgPatrolDetailData mfgPatrolDetailData:datas){
							System.out.println(sdf.format(mfgPatrolDetailData.getInspectionDate()));
							System.out.println(date);
							if(sdf.format(mfgPatrolDetailData.getInspectionDate()).equals(date)){								
				...
				<td style="padding:0px;" class="checkItemsClass" itemName="<%=checkItem.getCheckItemName()%>"  tdShow="<%=isShow%>"  tdFlag="<%=m %>" records="true">
					...
					<%
						if(InspectingItem.COUNTTYPE_COUNT.equals(checkItem.getCountType())){
					%>
						<textarea style="width:98%;height:100%;" rows=3 result=true fieldName="results" name="a<%=flag %>_results_<%=date%>" class="{maxlength:1000}"><%=data.getResults()==null?"":data.getResults() %></textarea>	
					<%		
					}else{ 
							Map<String,Double> tempMap = resultMap.get(checkItem.getId());						 	
							int spaceCount=5;
							if(tempMap.keySet().size()>0&&dateSet.size()>0){
							...
							for(int j=0;j<count;j++){
								String name = "result"+(j+1);
								Double value = tempMap.get(date==null?"":date.toString()+name);
								String color = "black";
