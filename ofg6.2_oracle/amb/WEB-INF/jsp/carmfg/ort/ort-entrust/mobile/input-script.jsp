...
function copyPersonCallBack(selectHiddenValue) {
	...
	$("#applicationForm").attr("action", "${mfgctx}/ort/ort-entrust/copytask.htm?copyPerson=" + selectHiddenValue);
	$("#applicationForm").ajaxSubmit(function(id) {
		alert(id);
	});
}
function assignUser(selectHiddenValue){
	var url ="${mfgctx}/ort/ort-entrust/assign.htm?assignee="+selectHiddenValue;
...
// 		}
		...
		$("#applicationForm").attr("action","${mfgctx}/ort/ort-entrust/save.htm");
	...
	function submitForm(){
		...
		$("#applicationForm").attr("action",'${mfgctx}/ort/ort-entrust/submit-process.htm');
		$("#applicationForm :input[name]").removeAttr("disabled");
		$("#applicationForm").submit();
	}
	...
	function _completeTask(taskTransact) {
		...
		$("#applicationForm :input[name]").removeAttr("disabled");
		$("#applicationForm").attr("action","${mfgctx}/ort/ort-entrust/complete-task.htm");
		var validator = $("#applicationForm").data('validator');
		if(validator){
			validator.cancelSubmit = true;
		}
