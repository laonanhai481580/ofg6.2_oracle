...
function addExceptionSingle(obj){
	...
	if(!id){
		...
		window.open('${epmctx}/exception-single/input.htm?type=ort&&formId='+id);
	...
	function selectProject(obj){
		...
		$.colorbox({href:"${epmctx}/base-info/ort-item/select-list.htm?customerNo="+customerNo+"&productNo="+productNo+"&sampleType="+sampleType,
			iframe:true, 
	...
	function alterException(obj){
		...
		$.post('${epmctx}/entrust-ort/input.htm?id='+obj+"&&type=ort&&str="+str,
			function(data){
	...
	function subControl(a){
		...
		if(!str){
			...
			$("input:[fieldname='defectNumber']").each(function(index, obj) {
	...
	function Epm(){
		$.colorbox({href:"${epmctx}/base-info/ort-item/list-view.htm",iframe:true,
...
// 		var b=idFirst.slice(2);
	...
	function exceptionInput(exceptionId){
		if(exceptionId&&exceptionId!=""){
			window.open('${epmctx}/exception-single/input.htm?id='+exceptionId);			
		};		
	}	
