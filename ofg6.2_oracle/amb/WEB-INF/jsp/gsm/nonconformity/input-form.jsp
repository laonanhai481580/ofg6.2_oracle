...
<%-- 			<input type="hidden" id="" name="" value="${ }"> --%>
		...
		<tr>
			...
			<td>
	        	...
	        	<input type="hidden" login="true" name="userPeopleLogin" id="userPeopleLogin"  value="${userPeopleLogin}" />
             	<a class="small-button-bg" style="margin-left:2px;" onclick="clearValue('userPeople','userPeopleLogin')" href="javascript:void(0);" title="<s:text name='清空'/>"><span class="ui-icon ui-icon-trash" style='cursor:pointer;'></span>
			</td>
			<td >使用部门确认:</td>
			<td colspan="2"><textarea id="userPeopleOpinion" name="userPeopleOpinion" >${userPeopleOpinion }</textarea></td>
		</tr>
		<tr id="jointlySignStr">
			...
			<td >工程部意见:</td>
			<td colspan="2"><textarea id="EPCdeptOpinion" name="EPCdeptOpinion" >${EPCdeptOpinion }</textarea></td>
		...
		<tr>
			...
			<a class="small-button-bg" style="margin-left:2px;" onclick="clearValue('QAdept','QAdeptLogin')" href="javascript:void(0);" title="<s:text name='清空'/>"><span class="ui-icon ui-icon-trash" style='cursor:pointer;'></span>
			</td>
			<td >品保部意见:</td>
			<td colspan="2"><textarea id="QAdeptOpinion" name="QAdeptOpinion" >${QAdeptOpinion }</textarea></td>
		</tr>
		<tr>
			<td colspan="2"></td>
			<td>是否需要重新检测</td>
			<td colspan="2">
				<input  type="radio" id="d1" name="anewRetest" value="是"  <s:if test='%{anewRetest=="是"}'>checked="true"</s:if> title="是"/><label for="d1">是</label>
				<input  type="radio" id="d2" name="anewRetest" value="否"  <s:if test='%{anewRetest=="否"}'>checked="true"</s:if> title="否"/><label for="d2">否</label>
			</td>
