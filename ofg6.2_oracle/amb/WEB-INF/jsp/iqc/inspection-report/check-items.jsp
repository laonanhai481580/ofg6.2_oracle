...
<!-- 			<td style="width:80px;text-align:center;border-top:0px;">操作</td> -->
			...
			<td style="width:200px;text-align:center;border-top:0px;">检查方法</td>
			...
			<td style="width:70px;text-align: center;border-top:0px;">允收比例(%)</td>
			<td style="width:40px;text-align: center;border-top:0px;">合格数</td>
			<td style="width:40px;text-align: center;border-top:0px;">不良数</td>
			<td style="width:60px;text-align: center;border-top:0px;">结论</td>
...
<%-- 					<input type="hidden" fieldName="inspectionMan" name="a<%=flag %>_inspectionMan" inspectionMan="<%=checkItem.getCheckItemName()%>" value="<%=checkItem.getInspectionMan()==null?"":checkItem.getInspectionMan()%>"/> --%>
					...
					<input type="hidden" fieldName="orderNum"  name="a<%=flag %>_orderNum" value="<%=checkItem.getOrderNum()==null?"":checkItem.getOrderNum()%>"/>
				...
				<td id="check-items-td" style="padding:0px;"  tdShow="<%=isShow%>" class="checkItemsClass" itemName="<%=checkItem.getCheckItemName()%>">
					<%
						if(InspectingItem.COUNTTYPE_COUNT.equals(checkItem.getCountType())){
					%> <textarea style="width:80%;height:80%;" rows=2 fieldName="results" name="a<%=flag %>_results"  class="{maxlength:1000}"><%=checkItem.getResults()==null||checkItem.getResults()==""?"合格":checkItem.getResults() %></textarea>
						<a class="small-button-bg" style="margin-left:2px;float:right;margin-top: 2px;" onclick="removeBadNum(this)" href="#" title="减少不合格数"><span class="ui-icon ui-icon-minus" style='cursor:pointer;'></span></a>
						<a class="small-button-bg" style="margin-left:2px;float:right;margin-bottom: 2px;" onclick="addBadNum(this)" href="#" title="增加不合格数"><span class="ui-icon ui-icon-plus" style='cursor:pointer;'></span></a>
					<%}else{ 
							Map<String,Double> tempMap = resultMap.get(checkItem.getId());
							/* int count=0;
							if(!"欧菲光-神奇工场".equals(ContextUtils.getCompanyName())){
				...
				<td style="text-align: center;"  tdShow="<%=isShow%>" itemName="<%=checkItem.getCheckItemName()%>">
					<input type="hidden" style="width:95%;padding:0px;text-align:center;" hisParent="<%=checkItem.getInspectionType() %>" fieldName="allowProportion" name="a<%=flag %>_allowProportion" value="<%=checkItem.getAllowProportion()==null?"":checkItem.getAllowProportion()%>" hisamount="<%=checkItem.getAllowProportion()==null?"0":checkItem.getAllowProportion() %>"  ></input>
					<%=checkItem.getAllowProportion()==null?"":checkItem.getAllowProportion() %>
				</td>
				<td style="text-align: center;"  tdShow="<%=isShow%>" itemName="<%=checkItem.getCheckItemName()%>">
