...
<%-- 			<%=ContextUtils.getSubCompanyName()==null?"":"("+ContextUtils.getSubCompanyName()+")"%> --%>
		...
		<tr>
			...
			<td>
			    <select name="processSection" id="processSection" class="xla_k">
                         <option value=""></option>
                         ...
                         <option value="1号园区IQC" <c:if test='${"1号园区IQC" eq processSection}'>selected</c:if>>1号园区IQC</option>
                </select>
		...
		<tr>
			...
			<td>
				...
				<%}else{ %>
<%-- 					<%=df.format(val) %>${units} --%>
					<input name="stockAmount" id="stockAmount" value="<%=df.format(val) %>"  class="{required:true,number:true,messages:{required:'来料数量不能为空'}}"></input>
				<%}%> 
...
<%-- 				<input name="inspectionConclusion" id="inspectionConclusion" value="${inspectionConclusion}" type="hidden"/> --%>
				<select name="inspectionConclusion" id="inspectionConclusion" class="xla_k">
                         <option value=""></option>
