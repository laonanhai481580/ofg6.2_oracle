...
function lastAuditIqc(obj){
	...
	var reportId = obj.id;
	var checkBomCode = obj.checkBomCode;
	...
	var processingResult = obj.processingResult;
	var sqeMrbReportNo = obj.sqeMrbReportNo;
	...
	html += '<tr height=29><td style="width:20%;">检验物料</td><td  style="width:10%;"><input value="'+reportId+'" type="hidden" name="id"></input><input value="'+checkBomCode+'" disabled="disabled"></input></td>';
	...
	html += '<tr><td>处理方式</td><td><input name="processingResult"  value="'+processingResult+'"/></td>';
	html += '<td>Mrb单号</td><td><input name="sqeMrbReportNo"  value="'+sqeMrbReportNo+'"/></td>';
	html += '<td>退货单号</td><td><input name="returnReportNo"  value="'+returnReportNo+'"/></td></tr>';
	html += '<tr><td>SQE确认意见</td><td colspan="5"><textarea name="lastStateText" rows="3" cols="3"></textarea></td></tr>';
	html += '</table></form>';
