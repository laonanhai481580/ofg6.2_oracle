		...
		$.upload({
			...
			callback:function(files){
				if(files.length>0){
					...
					$.showMessage("正在保存监控方案,请稍候... ...","custom");
					$.post("${spcctx}/base-info/monitor-program/save-attach.htm?attachUrl="+files[0].id+"&&id="+monitorProgramId, {}, function(result) {
						$("button").removeAttr("disabled","disabled");
						$.clearMessage();
						if(result.error){
							alert(result.message);
						}else{
							$.showMessage("保存成功,正在刷新... ...");
...
<img  width="100%" id="monitorImage" style="position:absolute;left:0px;top:0px;z-index:1" onload="afterLoadImage();" onerror="afterLoadImage();"></img>
<div id="test1"></div>	
<!-- 添加的菜单 -->
