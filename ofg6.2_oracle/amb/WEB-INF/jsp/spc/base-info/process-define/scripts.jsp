...
<script type="text/javascript">
	...
	//检查历史数据并生成对应异常消息
	function lanchAbnormalHis(){
		var featureId=$("#id").val();
		var num=1;
 		if(!confirm("确定执行操作吗?")){
 			return;
 		}
 		if($(":input[name=name]").val()==""){
			alert("检查历史数据错误，请先选择某个质量特性！");
			return;
		}
 		var url = '${spcctx}/process-monitor/lanch-abnormal.htm';
 		$.showMessage("正在执行操作,请稍候...","custom");
 		$.post(url,{featureId:featureId,num:num,queryHist:1},function(result){
 			$.clearMessage();
 			if(result!=null){
 				alert("检查历史数据错误!");
 			}else{
 				$.showMessage("执行成功.");//,"custom"
 			}
 		},'json');
	}
	//规格类型变化
	...
	function submitForm(url){
		if($('#contentForm').valid()){
			...
			for(var i=0;i<rows2.length;i++){
				...
				nos.push(rows2[i].detailCode);
			}
			params.levelIds = ids.join(",");
			params.detailCodes = nos.join(",");
			var paras = new Array();
...
// 			alert($.param(params));
	...
	function setPerson(){
		...
		for(var i=0;i<=mydata.length;i++){
			jQuery("#person_table").jqGrid('addRowData',mydata[i].code,mydata[i]);
		}
	}
	//删除异常通知人员
	function delPerson() {
