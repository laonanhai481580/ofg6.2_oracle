...
<script type="text/javascript">
	...
	function setSupplierValue(objs){
		...
		$("#sqeDepartmentName").val(obj.shareGroup);
			...
			$("#left_tree_div").jstree({ 
				...
				if(isLeaf == 'true'){
					...
					if(customType == 'estimateModel'){
						...
						<security:authorize ifAnyGranted="supplier_evaluate_ledger">
						$(document).mask();
						window.location = '${supplierctx}/evaluate/quarter/ledger.htm?supplierId=${supplierId}&evaluateYear=${evaluateYear}&modelId='+modelId;
						</security:authorize>
						...
						<security:authorize ifAnyGranted="supplier_evaluate_total-table">
						$(document).mask();
						window.location = '${supplierctx}/evaluate/quarter/evaluate-total-table.htm?supplierId=${supplierId}&evaluateYear=${evaluateYear}&modelId=${modelId}';
						</security:authorize>
					};
				};
			});
...
<div id="accordion1">
	...
	<security:authorize ifAnyGranted="supplier_evaluate_manager_warning-sign,supplier_evaluate_manager_estimate-stat-distribution,supplier_evaluate_manager_qcds,supplier_evaluate_point-rank,supplier_evaluate_total-view">
		<h3><a id="_list" onclick="javascript:window.location='<grid:authorize code="supplier_evaluate_manager_warning-sign,supplier_evaluate_manager_estimate-stat-distribution,supplier_evaluate_manager_qcds,supplier_evaluate_point-rank,supplier_evaluate_total-view" systemCode="supplier"></grid:authorize>';">评价汇总</a></h3>
		<div style="padding:0px;">
			<!-- 供应商红黄牌 -->
...
<%-- 			</security:authorize> --%>
			...
			<security:authorize ifAnyGranted="supplier_evaluate_improve-view">
				<div id="total_improve_list" class="west-notree" onclick="javascript:window.location='${supplierctx}/evaluate/quarter/list-improve.htm';"><span>供应商评价改进表</span></div>
			</security:authorize>
		</div>
	</security:authorize>
</div>
