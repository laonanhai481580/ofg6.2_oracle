...
<script type="text/javascript">
	...
	function openexceptionImprove(id){
		var url= '${supplierctx}/improve-new/exception-improve/input.htm?id=' + id;
		window.open(url);
	}
	...
	function addImprove(){
		...
		if(isimprove=="true"){
			...
			window.open('${supplierctx}/improve-new/exception-improve/input.htm?formId=${id}&&type=');	
		}
	}	
	function isImproveChange(obj){
		var isImprove=$("#isImprove").val();
		if(isImprove=="需要"){
			$("#improveBtn").show();
		}else{
	...
	function signMessage(obj){
		var isImprove=$("#isImprove").val();
		var exceptionImproveId=$("#exceptionImproveId").val();
		if(isImprove=="需要"){
			if(exceptionImproveId){
				$("#signTd").html("以下内容请在物料异常矫正单中办理");
			}else{
				$("#signTd").html("请发起物料异常矫正单");
				$("#improveBtn").show();	
			}
			$("#signTr").show();
