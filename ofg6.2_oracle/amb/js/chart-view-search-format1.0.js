...
//添加IQC物料名称方法
window.chartView.addSearchFormat({
	code:'iqcMaterielName',
	name:'IQC-物料名称',
	initFormat:function($input){
		//弹出框
		var id = $input.attr("id");
		var $a = $('<a class="small-button-bg" autoAppend=true id="'+id+'SelectA" style="float:left;margin-left:2px;margin-top:1px;" onclick="window.chartView._selectIqcMaterielNameClick(\''+id+'\')" href="javascript:void(0);" title="选择物料"><span class="ui-icon ui-icon-search" style="cursor:pointer;"></span></a>');
		$input.css({
			"float":"left",
			"margin-top":"2px"
		}).width($input.width()-22)
...
//添加IQC物料编码方法
window.chartView.addSearchFormat({
	code:'iqcMaterielCode',
	name:'IQC-物料编码',
	initFormat:function($input){
		//弹出框
		var id = $input.attr("id");
		var $a = $('<a class="small-button-bg" autoAppend=true id="'+id+'SelectA" style="float:left;margin-left:2px;margin-top:1px;" onclick="window.chartView._selectIqcMaterielCodeClick(\''+id+'\')" href="javascript:void(0);" title="选择物料"><span class="ui-icon ui-icon-search" style="cursor:pointer;"></span></a>');
		$input.css({
			"float":"left",
			"margin-top":"2px"
		}).width($input.width()-22)
...
//选择IQC物料名称
window.chartView._selectIqcMaterielNameClick = function(inputId){
	window.chartView._selectObjId = inputId;
	window.chartView._fieldType="iqc-name";
	var webRoot = window.chartView.webRoot;
	if(!webRoot){
		webRoot = window.customSearchView.webRoot;
	}
	var selectIqcMaterielCodeClick = webRoot + "/iqc/statistical-analysis/bom-code-select.htm";
	$.colorbox({href:selectIqcMaterielCodeClick,iframe:true,
...
//选择IQC物料编码
window.chartView._selectIqcMaterielCodeClick = function(inputId){
	window.chartView._selectObjId = inputId;
	window.chartView._fieldType="iqc-code";
	var webRoot = window.chartView.webRoot;
	if(!webRoot){
		webRoot = window.customSearchView.webRoot;
	}
...
//选择IQC物料名称后的回写
function setIqcBomValue(objs){
	var names = "",codes="";	
	/*if(window.chartView._fieldType=="iqc-code"){
		...
		$("#" + window.chartView._selectObjId).val(codes);
	}
	if(window.chartView._fieldType=="iqc-name"){
		for(var i=0;i<objs.length;i++){
			if(names.length==0){
				names = objs[i].name;
			}else{
	...
	if(window.chartView._fieldType=="iqc-code"){
		...
		$("#" + window.chartView._selectObjId).val(names);
		//$("#" + window.chartView._selectObjId).val(objs[0].code);
	}
	if(window.chartView._fieldType=="iqc-name"){
		var names = "";
		for(var i=0;i<objs.length;i++){
...
function setMaterialTypeValue(objs){
	...
	$("#" + window.chartView._selectObjId).val(names);
	//$("#" + window.chartView._selectObjId).val(objs[0].materialType);
};
//添加机种的格式化方法
window.chartView.addSearchFormat({
	code:'productName',
...
function setProductValues(objs){
	...
	$("#" + window.chartView._selectObjId).val(names);*/
	$("#" + window.chartView._selectObjId).val(objs[0].machineNo);
};
...
function setOqcProductValues(objs){
	...
	$("#" + window.chartView._selectObjId).val(names);
	//$("#" + window.chartView._selectObjId).val(objs[0].model);
};
...
function setcustomerValues(objs){
	...
	$("#" + window.chartView._selectObjId).val(names);
	//$("#" + window.chartView._selectObjId).val(objs[0].customerName);
};
//添加机型格式化方法
window.chartView.addSearchFormat({
	code:'modelClick',
