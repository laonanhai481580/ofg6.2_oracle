								...
								for (var key in result) { 
									if(key!='fileId'&&key!="fileName"){
										...
										for(var m=1;m<=datasArr.length;m++){
											 $("#checkItemsParent td[itemname="+key+"]").find(":input[fieldname=result"+m+"]").val(parseFloat(datasArr[m-1]));
...
//					.css("background","blue")
			...
			function appendFiles(files){
				...
				for(var i=0;i<files.length;i++){
					var file = files[i];
					var FileListType="jpg,JPG,gif,GIF,jpeg,JPEG,png,PNG,txt,xls,xlsx,pdf,docx,dot,doc,pptx,zip,ZIP,exe,rar,RAR";
					...
					if(FileListType.indexOf(destStr) == -1){
						  alert("只允许上传jpg,JPG,gif,GIF,jpeg,JPEG,png,PNG,txt,xls,xlsx,pdf,docx,dot,doc,pptx,zip,ZIP,exe,rar,RAR文件");
						  return ;
					}
