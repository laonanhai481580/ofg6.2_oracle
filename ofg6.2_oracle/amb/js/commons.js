...
$.extend($.jgrid.defaults,{
	beforeRequest : function(){
		...
		if(postData&&postData.searchParameters){
			//处理下拉框不能模糊查询的原因
			try {
				var parameters = $.parseJSON(postData.searchParameters);
				for(var i=0;i<parameters.length;i++){
					var param = parameters[i];
					if($(":input[name="+param.propName+"]").is("select")){
						param.optSign = "=";
					}
				}
				postData.searchParameters = JSON.stringify(parameters);
			} catch (e) {
				//不规则的查询参数格式时不处理
			}
		}
...
$.extend({
	...
	getDownloadHtml: function(val){
		...
		for(var i=0;i<files.length;i++){
			...
			html += '<a style="text-decoration:underline;" href="'+$.getDownloadPath(file.id)+'" title="下载'+file.fileName+'">'+file.fileName+'</a>';
		}
		return html;
	},
	...
	upload:function(params){
					...
					click : function(){
						...
						$("#" + windowId).find("table tr").each(function(index,obj){
							...
							if(id){
								var file = {
									id : id,
									fileName : $(obj).find("input[name=fileName]").val()
								};
								files.push(file);
								if(fileStrs){
									fileStrs += "s~s";
								}
								fileStrs += file.id + '|~|' + file.fileName;
							}
						});
		...
		function init(){
			...
			if("iqcInspectionDatas"==params.showInputId){
				uploadUrl += "&iqcDatas=true";
			}else{
				uploadUrl += "&iqcDatas=false";
			}
			var settings = {
					...
					file_queued_handler : function(fileObj){
						appendFiles([{uploadId:fileObj.id,id:'',fileName:fileObj.name}]);
					},
								...
								for (var key in result) { 
									if(key!='fileId'&&key!="fileName"){
										...
										for(var m=1;m<=datasArr.length;m++){
											 if(isNaN(datasArr[m-1])){
												 $("#checkItemsParent td[itemname="+key+"]").find(":input[fieldname=results]").val(datasArr[m-1]);
											 }else{
												 $("#checkItemsParent td[itemname="+key+"]").find(":input[fieldname=result"+m+"]").val(datasArr[m-1]);
										    	   resultChangeSelf($("td[itemname="+key+"]").find(":input[fieldname=result"+m+"]"),datasArr[m-1]);
											 }
...
//					.css("background","blue")
			...
			function appendFiles(files){
				...
				for(var i=0;i<files.length;i++){
					var file = files[i];
					var FileListType="jpg,JPG,gif,GIF,jpeg,JPEG,png,PNG,txt,xls,xlsx,pdf,PDF,docx,dot,doc,pptx,zip,ZIP,rar,RAR,exe,rar,RAR";
					var fileTrueName = file.fileName;
					var destStr = fileTrueName.substring(fileTrueName.lastIndexOf(".")+1,fileTrueName.length);
					if(FileListType.indexOf(destStr) == -1){
						  alert("只允许上传jpg,JPG,gif,GIF,jpeg,JPEG,png,PNG,txt,xls,xlsx,pdf,PDF,docx,dot,doc,pptx,zip,ZIP,exe,rar,RAR文件");
						  return ;
					}
					var trId = "file_" + (file.uploadId?file.uploadId:file.id);
					$("#" + windowId).find(".body").append('<tr height=29 id="'+trId+'">'
							+ '<td style="padding:0px 0px 0px 4px;margin:0px;border-bottom:1px dotted gray;"><span>'+file.fileName+'</span><input type="hidden" name="fileName" value="'+file.fileName+'"></input>'
							+ '<input type="hidden" name="uploadId" value="'+(file.uploadId?file.uploadId:'')+'"></input><input type="hidden" name="id" value="'+(file.id?file.id:'')+'"></input></td>'
							+ '<td style="width:70px;border-bottom:1px dotted gray;">'+(file.uploadId?'<a style="color:red;" href="#" uploadId="'+file.uploadId+'" class="upload" title="上传'+file.fileName+'">上传</a>':'<a href="'+$.getDownloadPath(file.id)+'" title="上传'+file.fileName+'">下载</a>') + ' <a href="#" class="clear" title="清除'+file.fileName+'">清除</a></td>'
							+ '</tr>');
					...
					var operateTd = 
						tr$.find("td:last");
					operateTd.find("a.clear").click(function(){
						$("#" + windowId + " .edit").hide();
			...
			function uploadFile(tr$){
				$("#fileProgressbar")
					...
					.progressbar('value',[0]);
				var p = tr$.position();
				$("#fileProgressbar").css("top",(p.top+14) + "px")
					.css("left",p.left + "px")
					.width(tr$.width());
				$("#fileProgressbar").attr("targetTrId",tr$.attr("id"));
				if(window.applicationCache){
					$.swfu.upload(tr$.find("input[name=uploadId]").val());
				}else{
					$.swfu.setPostParams({
			...
			function uploadFirstFile(){
				$("#" + windowId).find(".body tr").each(function(index,obj){
					...
					var FileListType="jpg,JPG,gif,GIF,jpeg,JPEG,png,PNG,txt,xls,xlsx,pdf,PDF,docx,dot,doc,pptx,zip,ZIP,exe,rar,RAR";
					var fileTrueName = $(obj).find("input[name=fileName]").val();
					var destStr = fileTrueName.substring(fileTrueName.lastIndexOf(".")+1,fileTrueName.length);
					if(FileListType.indexOf(destStr) == -1){
						  alert("只允许上传jpg,JPG,gif,GIF,jpeg,JPEG,png,PNG,txt,xls,xlsx,pdf,PDF,docx,dot,doc,pptx,zip,ZIP,exe,rar,RAR文件");
						  return ;
	...
	if(workflowInfoState=="MANUAL_END"){
		...
		if(launchState){
			...
			if(launchState.indexOf(colName)>-1){
				...
				if(reg.test(launchState)&&changeWorkFlowColor!=undefined&&changeWorkFlowColor.indexOf('red')>-1){
					return "<div style='text-align:center;margin-left:-10px;color:red;'>办理中...</div>";
				}else if(reg.test(launchState)){
					return "<div style='text-align:center;margin-left:-10px;color:green;'>办理中...</div>";
				}else if(changeWorkFlowColor!=undefined&&changeWorkFlowColor.indexOf(colName)>-1){
					return "<div style='text-align:center;margin-left:-10px;'><img src='"+webRoot+"/images/red.gif'/></div>";					
				}else{
