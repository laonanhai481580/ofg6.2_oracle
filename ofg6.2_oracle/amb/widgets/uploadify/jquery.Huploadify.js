(function($) {
	$.fn.Huploadify = function(opts) {
		var itemTemplate = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><span class="up_filename">${fileName}</span><a href="javascript:void(0);" class="uploadbtn">上传</a><a href="javascript:void(0);" class="delfilebtn">删除</a></div>';
		var defaults = {
			...
			itemTemplate : itemTemplate, //上传队列显示的模板
			onUploadStart : null, //上传开始时的动作
			onUploadProgress : null,//上传进度条的动作
		...
		this.each(function(index, element) {
			...
			var uploadManager = {
				...
				init : function() {
					var inputStr = '<div style="float:left;width:70px;margin-top:2px;margin-right:6px;overflow:hidden;background:url(/amb/images/selectFile.jpg) no-repeat -5px -3px;" class="ui-button ui-widget ui-state-default ui-corner-all ui-button-text-only"><input id="select_btn_' + instanceNumber + '" class="selectbtn" style="outline: none;border: rgba(255,255,255,0) 1px solid !important;width: 75px;height:20px;margin-left:-10px;opacity: 0;filter: Alpha(opacity=0);" type="file" name="fileselect[]"';
					inputStr += option.multiple ? ' multiple' : '';
					inputStr += ' accept="';
					inputStr += F.getFileTypes(option.fileTypeExts).join(",");
					inputStr += '"/></div>';
					inputStr += '<a style="color: #FFF;text-decoration: none;display:none;" id="file_upload_' + instanceNumber + '-button" href="javascript:void(0)" class="uploadify-button">';
				...
				_uploadFile : function(file) {
					...
					if (xhr.upload) {
						...
						xhr.upload.onprogress = function(e) {
							...
							option.onUploadProgress && option.onUploadProgress(file, e.loaded, e.total);
						};
						...
						if (file.status === 0) {
							...
							fd.append(option.fileObjName+"Name", file.name);
							fd.append(option.fileObjName+"Type", file.type);
							if (option.formData) {
