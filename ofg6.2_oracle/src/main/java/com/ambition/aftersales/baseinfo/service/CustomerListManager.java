...
public class CustomerListManager {
	...
	//验证并保存记录
	public boolean isExistCustomer(String customerName){
		String hql = "select count(*) from CustomerList d where d.companyId =?  and d.customerName = ? ";
		List<Object> params = new ArrayList<Object>();
		params.add(ContextUtils.getCompanyId());
	...
	//删除记录
	public String deleteCustomerList(String ids){
		...
		for(String id : deleteIds){
			...
			sb.append(customerListDao.get(Long.valueOf(id)).getCustomerName() + ",");
		}
		return sb.toString();
	}
