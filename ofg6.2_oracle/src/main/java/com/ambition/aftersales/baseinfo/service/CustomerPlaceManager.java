...
public class CustomerPlaceManager {
	...
	//删除记录
	public String deleteCustomerPlace(String ids){
		...
		for(String id : deleteIds){
			...
			sb.append(customerPlaceDao.get(Long.valueOf(id)).getCustomerPlace() + ",");
		}
		return sb.toString();
	}
	//删除对象
...
//	返回页面对象列表
    ...
    */
	...
	public List<Option> listAllForOptions(){
		List<CustomerPlace> customerPlaces = listPlace();
		List<Option> options = new ArrayList<Option>();
		for(CustomerPlace customerPlace : customerPlaces){
			Option option = new Option();
			option.setName(customerPlace.getCustomerPlace());
			option.setValue(customerPlace.getCustomerPlace());
			options.add(option);
		}
		return options;
	}
