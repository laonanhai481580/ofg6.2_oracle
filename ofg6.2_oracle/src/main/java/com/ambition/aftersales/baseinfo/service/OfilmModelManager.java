...
public class OfilmModelManager {
	...
	//验证并保存记录
	public boolean isExistCustomerModel(String customerName,String model){
		String hql = "select count(*) from OfilmModel d where d.companyId =?  and d.customerModel = ? and d.customerList.customerName=? ";
		List<Object> params = new ArrayList<Object>();
		params.add(ContextUtils.getCompanyId());
		params.add(model);
		params.add(customerName);
		Query query = ofilmModelDao.getSession().createQuery(hql);
	...
	//删除记录
	public String deleteOfilmModel(String ids){
		StringBuilder sb = new StringBuilder("");
		String[] deleteIds = ids.split(",");
		for(String id : deleteIds){
			ofilmModelDao.delete(Long.valueOf(id));
			sb.append(ofilmModelDao.get(Long.valueOf(id)).getOfilmModel() + ",");
		}
		return sb.toString();
	}
	//删除对象
