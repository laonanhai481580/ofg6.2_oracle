...
public class CustomerListAction extends BaseAction<CustomerList>{
	...
	protected void prepareModel() throws Exception {
		if(id == null){
			...
			customerList.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
		}else{
	...
	public String delete() throws Exception {
		try{
			String deleteNos=customerListManager.deleteCustomerList(deleteIds);
			//日志消息,方便跟踪删除记录
			Struts2Utils.getRequest().setAttribute(LogInfo.MESSAGE_ATTRIBUTE,"删除客户清单台帐,客户名称为【" + deleteNos + "】!");
		}catch(Exception e){
			e.printStackTrace();
		}
		return null;
	...
	public String save() throws Exception {
		...
		try{
			...
			//日志消息,方便跟踪删除记录
			Struts2Utils.getRequest().setAttribute(LogInfo.MESSAGE_ATTRIBUTE,"保存客户清单台帐,客户名称为【" + customerList.getCustomerName() + "】!");
