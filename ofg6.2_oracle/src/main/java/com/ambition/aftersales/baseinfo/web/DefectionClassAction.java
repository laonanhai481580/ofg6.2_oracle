...
public class DefectionClassAction extends BaseAction<DefectionClass>{
	...
	public String delete() throws Exception {
		try{
			String deleteNos=defectionClassManager.deleteDefectionClass(deleteIds);
			//日志消息,方便跟踪删除记录
			Struts2Utils.getRequest().setAttribute(LogInfo.MESSAGE_ATTRIBUTE,"删除不良类别台帐,不良类别为【" + deleteNos + "】!");
		}catch(Exception e){
			e.printStackTrace();
		}
		return null;
