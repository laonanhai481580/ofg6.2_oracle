...
public class OfilmModelAction extends BaseAction<OfilmModel>{
	...
	protected void prepareModel() throws Exception {
		if(id == null){
			...
			ofilmModel.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
		}else{
	...
	@Action("save")
	@LogInfo(optType="保存",message="欧菲机种")
	@Override
	...
	public String delete() throws Exception {
		String deleteNos=ofilmModelManager.deleteOfilmModel(deleteIds);
		//日志消息,方便跟踪删除记录
		Struts2Utils.getRequest().setAttribute(LogInfo.MESSAGE_ATTRIBUTE,"删除欧菲机种,【" + deleteNos + "】!");
		return null;
	}

	@Action("delete-subs")
	@LogInfo(optType="删除级联",message="欧菲机种维护")
	public String deleteSubs() throws Exception {
		...
		for(OfilmModel ofilmModel: list){
			...
			//日志消息,方便跟踪删除记录
			Struts2Utils.getRequest().setAttribute(LogInfo.MESSAGE_ATTRIBUTE,"删除欧菲机种,【" + ofilmModel.getOfilmModel() + "】!");
		}
		return null;
	}
