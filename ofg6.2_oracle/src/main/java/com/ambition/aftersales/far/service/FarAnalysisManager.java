...
public class FarAnalysisManager extends AmbWorkflowManagerBase<FarAnalysis>{
	...
	public Page<FarAnalysis> search(
		...
		return farAnalysisDao.search(page);
	}	
	
	public Page<FarAnalysis> searchHide(
			Page<FarAnalysis> page) {
		return farAnalysisDao.searchHide(page);
	}	
	...
	public String hiddenState(String hideId,String type){
		...
		for(String id : ids){
			FarAnalysis farAnalysis = farAnalysisDao.get(Long.valueOf(id));
			if("Y".equals(type)){
				farAnalysis.setHiddenState("N");
			}else{
				farAnalysis.setHiddenState("Y");
			}
			farAnalysisDao.save(farAnalysis);
			sb.append(farAnalysis.getFormNo() + ",");
		}
		return sb.toString();
	}	
	...
	public String deleteEntity(String ids) {
		...
		for (String id : deleteIds) {
			FarAnalysis report = getEntity(Long.valueOf(id));
