...
public class LarDataDao extends HibernateDao<LarData,Long>{
    ...
    public Page<LarData> search(Page<LarData> page) {
        return searchPageByHql(page, "from LarData d where d.companyId=? and d.hiddenState='N' ", ContextUtils.getCompanyId());
    }
    
    public Page<LarData> searchHide(Page<LarData> page) {
        return searchPageByHql(page, "from LarData d where d.companyId=? and d.hiddenState='Y' ", ContextUtils.getCompanyId());
    }   
}
