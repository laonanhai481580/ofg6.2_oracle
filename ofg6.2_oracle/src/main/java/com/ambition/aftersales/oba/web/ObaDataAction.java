...
public class ObaDataAction extends BaseAction<ObaData>{
	...
	@Action("delete")
	@LogInfo(optType="删除",message="删除OBA数据")
	...
	protected void prepareModel() throws Exception {
		if(id==null){
			...
			obaData.setObaDefectiveItems(new ArrayList<ObaDefectiveItem>());
			obaData.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
		}else{
...
//					generateHtml(userHtml,null, allDepartments, allUsers);
	...
	public String importDatas() throws Exception {
		try {
			if(myFile != null){
				...
				renderHtml(obaDataManager.importDatas(myFile,businessUnit));				
			}
		} catch (Exception e) {
	...
	@LogInfo(optType="下载",message="下载OBA数据导入模板")
	public String downloadTemplate() throws Exception {
		InputStream inputStream = null;
		try {
			inputStream = this.getClass().getClassLoader().getResourceAsStream("template/report/afs-oba-template.xlsx");
			Workbook book = WorkbookFactory.create(inputStream);
			String fileName = "OBA数据导入模板.xls";
	...
	public String inputNew() throws Exception {
	    ...
	    ActionContext.getContext().put("inspectionConclusions",ApiFactory.getSettingService().getOptionsByGroupCode("ok-ng"));
	    ActionContext.getContext().put("inspectionMethods",ApiFactory.getSettingService().getOptionsByGroupCode("afs-inspection-method"));
		List<Map<String,Object>> definitions = obaDataManager.queryDefectionsByBusinessUnit(businessUnit);
	...
	public String saveNew() throws Exception {
		...
		try{
			obaDataManager.saveNew(part,item);
