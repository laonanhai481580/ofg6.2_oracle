...
public class DefectionTypeDao extends HibernateDao<DefectionType,Long>{
	...
	public List<DefectionType> getDefectionTypeByBusinessUnit(String businessUnit,String productType){
		return find("from DefectionType d where d.companyId = ? and d.businessUnitName = ? and d.productType=? ", ContextUtils.getCompanyId(),businessUnit,productType);
	}
	public String getDefectionTypeNoByName(String name,String businessUnit){
		String hql = "from DefectionType d where d.defectionTypeName = ? and  d.businessUnitName= ? ";
	...
	public String getDefectionTypeNoByName(String name,String businessUnit,String productType){
		String hql = "from DefectionType d where d.defectionTypeName = ? and  d.businessUnitName= ? and d.productType=? ";
		List<DefectionType> defectionTypes = this.find(hql,name,businessUnit,productType);
		if(defectionTypes.isEmpty()){
			return null;
		}else{
			return defectionTypes.get(0).getDefectionTypeNo();
