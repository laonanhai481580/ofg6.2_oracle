...
public class DefectionCodeManager {
	...
	private boolean isExistDefectionCode(Long id, String no, String name, DefectionType defectionType) {
		String hql = "select count(*) from DefectionCode d where d.companyId = ?  and (d.defectionCodeNo =? or d.defectionCodeName =?) and d.defectionType.businessUnitName=? ";
		List<Object> params = new ArrayList<Object>();
		params.add(ContextUtils.getCompanyId());
		//params.add(defectionType);
		params.add(no);
		params.add(name);
		params.add(defectionType.getBusinessUnitName());
		if(defectionType.getProductType() != null){
			hql += " and d.defectionType.productType= ? ";
			params.add(defectionType.getProductType());
		}
		if(id != null){
			hql += " and d.id <> ?";
			params.add(id);
	...
	public void saveDefectionCode(DefectionCode defectionCode){
		...
		defectionCode.setDefectionCodeNo(defectionCode.getDefectionCodeNo().trim());
		...
		if(isExistDefectionCode(defectionCode.getId(),defectionCode.getDefectionCodeNo(),defectionCode.getDefectionCodeName(),defectionCode.getDefectionType())){
			throw new RuntimeException("产品类别"+defectionCode.getDefectionType().getProductType()+"下已存在相同的不良代码或名称!");
		}
	...
	//删除记录
	public String deleteDefectionCode(String ids){
		...
		for(String id : deleteIds){
			...
			sb.append(defectionCodeDao.get(Long.valueOf(id)).getDefectionCodeName()+ ",");
