...
public class MfgItemIndicatorManager {
	...
	public JSONObject copyInspectingIndicators(){
		...
		String workingProcedure=Struts2Utils.getParameter("workingProcedure");
		String standardVersion=Struts2Utils.getParameter("standardVersion");
		MfgInspectingIndicator mfgInspectingIndicator = mfgInspectingIndicatorDao.get(Long.valueOf(copyId));
		JSONObject obj = new JSONObject();
		if(mfgInspectingIndicator!=null){
			if(!isExistInspectingIndicator(null,model,workingProcedure,null)){
				//复制抽样的编码
				MfgInspectingIndicator targetInspectingIndicator = new MfgInspectingIndicator();
				targetInspectingIndicator.setWorkingProcedure(workingProcedure);
				targetInspectingIndicator.setModel(model);
				targetInspectingIndicator.setModelName(modelName);
				targetInspectingIndicator.setStandardVersion(standardVersion);
				...
				String businessUnitName=ContextUtils.getSubCompanyName();
				targetInspectingIndicator.setBusinessUnitName(businessUnitName);
				IndicatorAttach attach = indicatorAttachManager.copyIndicatorAttach(mfgInspectingIndicator.getIndicatorAttachId());
				if(attach != null){
				...
				for(MfgItemIndicator mfgItemIndicator : mfgInspectingIndicator.getMfgItemIndicators()){
					...
					targetIndicator.setInAmountFir(mfgItemIndicator.getInAmountFir());
					targetIndicator.setInAmountEnd(mfgItemIndicator.getInAmountEnd());
					targetIndicator.setInAmountPatrol(mfgItemIndicator.getInAmountPatrol());
					targetIndicator.setIsJnUnit(mfgItemIndicator.getIsJnUnit());
					targetIndicator.setCountType(mfgItemIndicator.getCountType());
					targetIndicator.setMethod(mfgItemIndicator.getMethod());
		...
		try {
			...
			for(int k=0;k<totalSheets;k++){
			    ...
			    for(int i = 0 ; i < sheetMergeCount ; i++ ){  
			 	...
			 	//机种
			    Cell businessUnitNameCell = ExcelUtil.getNextCell(sheet,cellMap.get("厂区"), cellRangeAddressMap);
			    String businessUnitName = null;
			    if(businessUnitNameCell != null){
			    	businessUnitName = (ExcelUtil.getCellValue(businessUnitNameCell)+"").trim();
			    }
			    if(businessUnitName==null||"".equals(businessUnitName)){
			    	businessUnitName=ContextUtils.getSubCompanyName();
			    }
				//机种
...
//				workingProcedure = workingProcedure.replaceAll(" ","");
				...
				while(rows.hasNext()){
					...
					if(cell != null){
						...
						specifications = CommonUtil.replaceSymbols(specifications);
					}
					//首件检验数量
						...
						if(StringUtils.isNotEmpty(values)){
							...
							if(qualityFeature != null){
	    						...
	    						for(FeatureLayer layer : qualityFeature.getFeatureLayers()){
	    							if(layer.getDetailName().equals("机台")&&StringUtils.isNotEmpty(mfgCheckInspectionReport.getProduceMachine())){
	    								layerMap.put(layer.getDetailCode(), mfgCheckInspectionReport.getProduceMachine());
