...
public class IpqcAuditDao extends HibernateDao<IpqcAudit,Long>{
	...
	public Page<IpqcAudit> searchHide(Page<IpqcAudit> page) {
		  String hql = "from IpqcAudit o where o.hiddenState='Y' ";
		  return searchPageByHql(page, hql);
	}
    public Page<IpqcAudit> search(Page<IpqcAudit> page,String businessUnit,String factory) {
        return searchPageByHql(page, "from IpqcAudit d where d.companyId=? and d.businessUnitName=? and d.factory=? ",ContextUtils.getCompanyId(),businessUnit,factory);
    }
    public Page<IpqcAudit> search(Page<IpqcAudit> page) {
        return searchPageByHql(page, "from IpqcAudit d where d.companyId=?  and d.hiddenState='N' and (d.isMiss='否' or d.isMiss is null) ",ContextUtils.getCompanyId());
    }
