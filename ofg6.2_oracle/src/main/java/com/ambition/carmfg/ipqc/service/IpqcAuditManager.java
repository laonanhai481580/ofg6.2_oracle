...
public class IpqcAuditManager {
	...
	public Page<IpqcAudit> search(Page<IpqcAudit>page,String businessUnit,String factory){
		return ipqcAuditDao.search(page, businessUnit,factory);
	}
	...
	public Page<IpqcAudit> searchHide(Page<IpqcAudit>page){
		return ipqcAuditDao.searchHide(page);		
	}	
	...
	public String hiddenState(String hideId,String type){
		...
		for(String id : ids){
			...
			if("Y".equals(type)){
				ipqcAudit.setHiddenState("N");
			}else{
				ipqcAudit.setHiddenState("Y");
			}
			ipqcAuditDao.save(ipqcAudit);
			sb.append(ipqcAudit.getAuditDate()+ ",");
		}
		return sb.toString();
	}
	...
	public void saveIpqcAudit(IpqcAudit ipqcAudit){
		    ...
		    boolean flag=true;
		    if(ContextUtils.getCompanyName().indexOf("CL")>-1){
		    	flag=false;
		    }
		    ...
		    /*if(!validateAuditCount(ipqcAudit.getAuditDate(),ipqcAudit.getBusinessUnitName(),ipqcAudit.getFactory(),ipqcAudit.getStation(),ipqcAudit.getClassGroup(),ipqcAudit.getAuditCount())){
		    	 throw new RuntimeException("同日期、厂区、工厂、工序、班别的稽核件数必须相同！");
		    }*/
		    validateAuditCount(ipqcAudit);
			ipqcAuditDao.save(ipqcAudit);
			//稽核预警
	...
	public void auditWarming(IpqcAudit ipqcAudit) {
		IpqcAuditWarming ipqcAuditWarming=ipqcAuditWarmingDao.serachWarming(ipqcAudit.getBusinessUnitName(),ipqcAudit.getFactory(), ipqcAudit.getStation(), ipqcAudit.getProblemDegree(), ipqcAudit.getAuditType(),ipqcAudit.getClassGroup());
		String warmingCycle=null;
		if(ipqcAuditWarming!=null){
			warmingCycle=ipqcAuditWarming.getWarmingCycle();
			String hql = "select d from IpqcAudit d where d.companyId = ? and d.businessUnitName = ?  and d.factory =? ";
			    List<Object> params = new ArrayList<Object>();
			    params.add(ContextUtils.getCompanyId());
			    params.add(ipqcAudit.getBusinessUnitName());
			    params.add(ipqcAudit.getFactory());
			    if (ipqcAuditWarming.getAuditType() != null) {
				      hql = hql + "and d.auditType = ? ";
				      params.add(ipqcAuditWarming.getAuditType());
				   }
			    if (ipqcAuditWarming.getStation() != null) {
				      hql = hql + "and d.station = ? ";
				      params.add(ipqcAuditWarming.getStation());
				   }
			    if (ipqcAuditWarming.getProblemDegree() != null) {
				      hql = hql + "and d.problemDegree = ? ";
				      params.add(ipqcAuditWarming.getProblemDegree());
				   }
			    if (ipqcAuditWarming.getClassGroup() != null) {
				      hql = hql + "and d.classGroup = ? ";
				      params.add(ipqcAuditWarming.getClassGroup());
				   }
			...
			List<IpqcAudit> list = query.list();
		    String message = "";
		    Integer repeatCount=ipqcAuditWarming.getRepeatCount();
		    String warmingManLogin=ipqcAuditWarming.getWarmingManLogin();
		    if (repeatCount!=null&&list.size() >=repeatCount){
		    	String a[]=warmingManLogin.split(",");
		    	for (int i = 0; i < a.length; i++) {
					String userId=a[i].toString();
					//发送邮件
					message = "*最新预警信息:【"+ipqcAudit.getBusinessUnitName()+"】厂区、" ;
					if (ipqcAuditWarming.getFactory() != null) {
						message+= "【"+ipqcAuditWarming.getFactory()+"】工厂、";
					   }
				    if (ipqcAuditWarming.getStation() != null) {
				    	message+= "【"+ipqcAuditWarming.getStation()+"】工序、";
					   }
				    if (ipqcAuditWarming.getClassGroup() != null) {
				    	message+= "【"+ipqcAuditWarming.getClassGroup()+"】班次、";
					   }
				    if (ipqcAuditWarming.getProblemDegree() != null) {
				    	message+= "问题严重度为【"+ipqcAuditWarming.getProblemDegree()+"】的";
					   }	
				    message+= "稽核类别为【"+ipqcAuditWarming.getAuditType()+"】的出现频率已超过预警线，请注意！";
				    message+="问题描述：";
				    for (int j = 1; j < list.size()+1; j++) {
				    	message+="【"+j+"】:"+list.get(j-1).getProblemDescribe()+";";
					}
					String email = ApiFactory.getAcsService().getUserByLoginName(userId).getEmail();
					if(StringUtils.isNotEmpty(email)){
						AsyncMailUtils.sendMail(email,"IPQC稽核预警",message);
					}
				}
		    }	
		}
				...
				if(StringUtils.isNotEmpty(personStrs)){
					personArray = JSONArray.fromObject(personStrs);											
						for(int i = 0;i<personArray.size();i++){
							...
							//发送邮件
							message = "*"+dateStr+"最新预警信息:"+ipqcAudit.getBusinessUnitName()+"，"+ipqcAudit.getAuditDate()+"IPQC稽核问题信息已录入，问题描述："+
									ipqcAudit.getProblemDescribe()+"，工序："+ipqcAudit.getStation()+"，班次："+ipqcAudit.getClassGroup()+"，问题严重度："+ipqcAudit.getProblemDegree()+",稽核员："+ipqcAudit.getAuditMan()+"，担当者："+ipqcAudit.getDutyMan()+".";
							String email = ApiFactory.getAcsService().getUserByLoginName(userId).getEmail();
							if(StringUtils.isNotEmpty(email)){
		...
		while(rows.hasNext()){
			...
			try {
				...
				for(String key : objMap.keySet()){
					CommonUtil.setProperty(ipqcAudit,key, objMap.get(key));
				}
				//根据问题严重度获取问题分数
				...
				String str=ipqcAudit.getAuditDate()+"_"+businessUnitName+"_"+ipqcAudit.getFactory()+"_"+ipqcAudit.getStation()+"_"+ipqcAudit.getClassGroup();
				if(map.containsKey(str)){
					if(ipqcAudit.getAuditCount()-map.get(str)!=0){
						throw new AmbFrameException("同日期、厂区、工厂、工序、班别的稽核件数必须相同！");
					}
				}else{
