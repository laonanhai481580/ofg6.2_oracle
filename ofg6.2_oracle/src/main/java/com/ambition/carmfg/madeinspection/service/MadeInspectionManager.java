	        ...
	        for(String id:deleteId){
	        	...
	        	if(ContextUtils.getUserName().equals(apply.getCreator())){
	        		...
	        		for (MfgCheckItem checkItem : checkItems) {
	        			...
	        			if(qualityFeature!=null&&spcSampleIds!=null&&qualityFeature!=null&&StringUtils.isNotEmpty(qualityFeature.getTargetTableName())){
    				...
    				if("计量".equals(countType) && StringUtils.isNotEmpty(featureId)){
    					...
    					if(valueList.size()>0){
    						...
    						for(FeatureLayer layer : qualityFeature.getFeatureLayers()){
    							if(layer.getDetailName().equals("机台")&&StringUtils.isNotEmpty(report.getProduceMachine())){
    								layerMap.put(layer.getDetailCode(), report.getProduceMachine());
    						...
    						List<String> list=spcImportManager.backImportValues(Long.valueOf(featureId),report.getInspectionDate(),null,null,hisIds,valueList, layerMap);
						...
						/*if(StringUtils.isNotEmpty(values)){
							...
							if(feature != null&&feature.getSampleCapacity()!=null){
								...
								spcSampleIds = mfgCheckInspectionReportManager.insertSpcByResults(feature,report.getInspectionDate(),hisSampleIds,values,report.getInspectionPointType(),report.getWorkGroupType(),report,checkItem.getEquipmentNo());
							}
...
//				}
     ...
     * @throws IOException
    */
   public void exportReport1(MfgCheckInspectionReport s) throws IOException{
   ...
   /**
	...
	public void exportReport(MfgCheckInspectionReport s) throws IOException{
		...
		try {
			...
			ExportExcelFormatter conclusionFormatter = new ExportExcelFormatter() {
				public String format(Object value, int rowNum, String fieldName,Cell cell) {
					...
					if("inspectionConclusion".equals(fieldName)){
						MfgCheckInspectionReport report = (MfgCheckInspectionReport)value;
			...
			//检验类型格式话
			ExportExcelFormatter inspectionPointTypeFormatter = new ExportExcelFormatter() {
				public String format(Object value, int rowNum, String fieldName,Cell cell) {
					InspectionPointTypeEnum inspectionPointType = null;
					if("inspectionPointType".equals(fieldName)){
						MfgCheckInspectionReport report = (MfgCheckInspectionReport)value;
						inspectionPointType = report.getInspectionPointType();
					}
			...
			formatterMap.put("inspectionPointType",inspectionPointTypeFormatter);
			//检验记录格式化
			formatterMap.put("checkItems.results",new ExportExcelFormatter() {
				public String format(Object value, int rowNum, String fieldName,Cell cell){
					String result = "";
					MfgCheckItem item = (MfgCheckItem)value;
	...
	//查询隐藏数据
		public Page<MfgCheckInspectionReport> listHid(Page<MfgCheckInspectionReport>page){
			return madeInspectionDao.listHid(page);
		}
	public void hiddenState(String hideId,String type){
		String[] ids = hideId.split(",");
		for(String id : ids){
			MfgCheckInspectionReport mfgCheckInspectionReport = madeInspectionDao.get(Long.valueOf(id));
			if("Y".equals(type)){
				mfgCheckInspectionReport.setHiddenState("N");
			}else{
				mfgCheckInspectionReport.setHiddenState("Y");
			}
			madeInspectionDao.save(mfgCheckInspectionReport);
		}
	}
