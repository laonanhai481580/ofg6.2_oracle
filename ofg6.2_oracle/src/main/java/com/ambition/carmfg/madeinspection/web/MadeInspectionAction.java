...
public class MadeInspectionAction extends  WorkflowActionSupport<MfgCheckInspectionReport> {
    ...
    @LogInfo(optType = "导出", message = "导出制程报告")
	...
	public String submitProcess() {
        ...
        try {
            ...
            if(isOver){
            	...
            	//邮件提醒责任人
            	sendEmail(mfgCheckInspectionReport);          		
            }else{
	...
	public void sendEmail(MfgCheckInspectionReport mfgCheckInspectionReport) {
		SimpleDateFormat sdf =   new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String inspectionDate = sdf.format(mfgCheckInspectionReport.getInspectionDate());
		String message = "*最新预警信息:【"+inspectionDate+"、"+mfgCheckInspectionReport.getProcessSection()+"】、" +"【"+mfgCheckInspectionReport.getFactory()+"】、机种为【"+mfgCheckInspectionReport.getMachineNo()+"】的检验单判定为NG，请注意！单号："+mfgCheckInspectionReport.getInspectionNo();
		if(mfgCheckInspectionReport.getDutyManLogin()!=null&&!"".equals(mfgCheckInspectionReport.getDutyManLogin())&&mfgCheckInspectionReport.getInspectionConclusion().equals("NG")){
			String email = ApiFactory.getAcsService().getUserByLoginName(mfgCheckInspectionReport.getDutyManLogin()).getEmail();
...
//        	mfgCheckInspectionReport.setInspectionDate(new Date());
        	...
        	if(copyId==null||"".equals(copyId)){
            	mfgCheckInspectionReport.setBusinessUnitName(CommonUtil.getDataByLogin());
            	mfgCheckInspectionReport.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));       	
        	}else{
        		MfgCheckInspectionReport report=madeInspectionManager.getMfgCheckInspectionReport(Long.valueOf(copyId));
        		mfgCheckInspectionReport.setBusinessUnitCode(report.getBusinessUnitCode());
        		mfgCheckInspectionReport.setBusinessUnitName(report.getBusinessUnitName());
        		mfgCheckInspectionReport.setInspectionPointType(report.getInspectionPointType());
        		mfgCheckInspectionReport.setProcessSection(report.getProcessSection());
        		mfgCheckInspectionReport.setFactory(report.getFactory());
        		mfgCheckInspectionReport.setWorkProcedure(report.getWorkProcedure());
        		mfgCheckInspectionReport.setSection(report.getSection());
        		mfgCheckInspectionReport.setProductModel(report.getProductModel());
        		mfgCheckInspectionReport.setProductStage(report.getProductStage());
        		mfgCheckInspectionReport.setMachineNo(report.getMachineNo());
        		mfgCheckInspectionReport.setStockAmount(report.getStockAmount());
        		mfgCheckInspectionReport.setWorkGroupType(report.getWorkGroupType());
        		mfgCheckInspectionReport.setProduceMachine(report.getProduceMachine());
        		mfgCheckInspectionReport.setProductionLine(report.getProductionLine());
        		mfgCheckInspectionReport.setChoiceAuditLoinMan(report.getChoiceAuditLoinMan());
        		mfgCheckInspectionReport.setChoiceAuditMan(report.getChoiceAuditMan());
        		mfgCheckInspectionReport.setAmountUnit(report.getAmountUnit());
        	}
        }
	}
	...
	public String hiddenState(){
		...
		madeInspectionManager.hiddenState(eid,type);
		return null;
	}
    @Action("list-hid")
	@LogInfo(optType="查询",message="查询隐藏数据")
	public String getListHid() throws Exception {
		try {
			page = madeInspectionManager.listHid(page);
		} catch (Exception e) {
