...
public class GsmEquipmentManager implements ComboxValues{
	...
	public List<GsmEquipment> listAll(String fac){
		return gsmEquipmentDao.getFacEquipment(fac);
	}
	/**
	 * 保存对象
	 * @param gsmEquipment
...
//			}
//		}
		if(isExistEquipment(gsmEquipment.getId(),gsmEquipment.getManagerAssets(),gsmEquipment.getBusinessUnits())){
			throw new RuntimeException("已存在相同的量检具管理编号!");
		}		
		gsmEquipmentDao.save(gsmEquipment);
	}
	//验证并保存记录
	public boolean isExistEquipment(Long id, String managerAssets,String businessUnits) {
		...
		if(businessUnits != null){
			hql += "and d.businessUnits= ?";
			params.add(businessUnits);
		}
	...
	public Page<GsmEquipment> getPageByGsmEquipment(Page<GsmEquipment>page,GsmEquipment gsmEquipment,String processSection){
		return gsmEquipmentDao.getPageByGsmEquipment(page,gsmEquipment,processSection);
	}
	public Page<GsmEquipment> getPageByGsmEquipmentSelect(Page<GsmEquipment>page){
		return gsmEquipmentDao.getPageByGsmEquipmentSelect(page);
	}
	public Page<GsmEquipment> getPageByGsmEquipmentSelectAll(Page<GsmEquipment>page){
		return gsmEquipmentDao.getPageByGsmEquipmentSelectAll(page);
	}
	/**
	  * 方法名:根据ID 取到盘点相关数据
	  * <p>功能说明：</p>
	  * @return
	 */
	public Page<GsmEquipment> getPageOfInventory(Page<GsmEquipment>page,String str1){
		return gsmEquipmentDao.getPageOfInventory(page,str1);
	}
	/**
	  * 方法名:根据登录名 取到盘点相关数据
	  * <p>功能说明：</p>
	  * @return
	 */
	public Page<GsmEquipment> getStockActual(Page<GsmEquipment>page,String str1,String name){
		return gsmEquipmentDao.getStockActual(page,str1,name);
	}
	/**
	  * 方法名:查询需转移人确认的数据
	  * <p>功能说明：</p>
	...
	public Page<GsmEquipment> getPageInTransfer(Page<GsmEquipment>page,String login){
		return gsmEquipmentDao.getPageInTransfer(page,login);
	}
	/**
	  * 方法名:转移台账数据
	  * <p>功能说明：</p>
	...
	public Page<GsmEquipment> getPageOfTransferDatas(Page<GsmEquipment>page,String login){
		return gsmEquipmentDao.getPageOfTransferDatas(page,login);
	}
    // 封装出货异常结果数据集的JSON格式
	...
	public Page<GsmEquipment> getPageByUseDept(Page<GsmEquipment>page,String useDept,String str){
		return gsmEquipmentDao.getPageByUseDept(page, useDept,str);
	}
		...
		for(String id : ids.split(",")){ 
			...
			if(list.size()>1){
				for(int i=0;list.size()>i;i++){
					if(gsmEquipment.getManagerAssets().equals(list.get(i).getManagerAssets())){
						if(InspectionPlan.STATE_CHECk_IN.equals(list.get(i).getInspectionState())||InspectionPlan.STATE_CHECk_WAIT.equals(list.get(i).getInspectionState())){
				...
				inspectionPlan.setProcessSection(gsmEquipment.getProcessSection());
				inspectionPlan.setBusinessUnits(gsmEquipment.getBusinessUnits());
				inspectionPlan.setHiddenState(gsmEquipment.getHiddenState());
				inspectionPlan.setFactoryClassify(gsmEquipment.getFactoryClassify());
	...
	//			gsmEquipment.setNextProofTime(new Date());
				...
				inspectionPlan.setGsmEquipment(gsmEquipment);
				inspectionPlan.setGsmState(gsmEquipment.getGsmState());
				if(gsmEquipment.getProcessSection()!=null){
				inspectionPlan.setProcessSection(gsmEquipment.getProcessSection());
				}
				if(gsmEquipment.getBusinessUnits()!=null){
					inspectionPlan.setBusinessUnits(gsmEquipment.getBusinessUnits());
				}
				inspectionPlan.setHiddenState(gsmEquipment.getHiddenState());
				inspectionPlan.setFactoryClassify(gsmEquipment.getFactoryClassify());
	...
	public InspectionPlan createEquipmentPlanZD(GsmEquipment gsmEquipment) throws Exception{ 
		...
		if(list.size()>0){
			...
			if("1".equals(a)){
				...
				inspectionPlan.setCompanyId(gsmEquipment.getCompanyId());
				inspectionPlan.setCreatedTime(new Date());
				inspectionPlan.setCreator("system");
				inspectionPlan.setCreatorName("system");
				inspectionPlan.setInspectionState(InspectionPlan.STATE_CHECk_WAIT);
				inspectionPlan.setYesOrNo(true);
				inspectionPlan.setAddress(gsmEquipment.getAddress());
				inspectionPlan.setProcessSection(gsmEquipment.getProcessSection());
...
//			}
			...
			inspectionMsaplan.setProcessSection(gsmEquipment.getProcessSection());
			inspectionMsaplan.setHiddenState(gsmEquipment.getHiddenState());
			inspectionMsaplan.setFactoryClassify(gsmEquipment.getFactoryClassify());
			if(gsmEquipment.getProofTime()!=null){
				inspectionMsaplan.setProofTime(gsmEquipment.getProofTime());
			}
			if(gsmEquipment.getNextProofTime()!=null){
				inspectionMsaplan.setNextProofTime(gsmEquipment.getNextProofTime());
			}
...
//			gsmEquipment.setSendState(gsmEquipmentMaintenance.getMaintenanceState());
	...
	public String saveGsmMailSettings(GsmMailSettings gsmMailSetting,String selFlag,Page<GsmEquipment> page){
		...
		if(selFlag!=null && selFlag.equals("select")){
			...
			page = getPageByGsmEquipment(page, null,null);
...
//			}
		...
		while(rows.hasNext()){
			...
			try {
				...
				if(list.isEmpty()){
					...
					gsmEquipment.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
					isNew = true;
				}else{
				...
				gsmEquipment.setFactoryClassify(subName);
				gsmEquipment.setGsmState("N");
				for(String key : objMap.keySet()){
					CommonUtil.setProperty(gsmEquipment,key, objMap.get(key));
				}				
	...
	public Page<GsmEquipment> listState(Page<GsmEquipment> page,String state,String str1){
		String hql = " from GsmEquipment e where e.gsmState=? ";
		List<Object> searchParams = new ArrayList<Object>();
		searchParams.add(state);
//		if("TP南昌1号园区".equals(str1)){
//			hql=hql+" and e.businessUnits=?";
//			searchParams.add("南昌园区");
//		}else if("TP苏州厂区".equals(str1)){
//			hql=hql+" and e.businessUnits=?";
//			searchParams.add("苏州园区");
//		}else if("TP深圳厂区".equals(str1)){
//			hql=hql+" and e.businessUnits=?";
//			searchParams.add("深圳园区");
//		}
		if(str1!=null&&str1.equals(str1)){
			hql=hql+" and e.factoryClassify=?";
			searchParams.add(str1);
		}
		return gsmEquipmentDao.searchPageByHql(page, hql, searchParams.toArray());
	}
	...
	public void updateStockData(String row) throws ParseException{
		String[] ids = row.split(",");
		String type= Struts2Utils.getParameter("type");
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		String stockPlanTime= Struts2Utils.getParameter("stockPlanTime");
		Date plan=null;
		Date actual=null;
		if(stockPlanTime!=null){
			plan=sdf.parse(stockPlanTime);
		}
		String stockPlanPerson= Struts2Utils.getParameter("stockPlanPerson");
		String stockActualTime= Struts2Utils.getParameter("stockActualTime");
		if(stockActualTime!=null){
			actual=sdf.parse(stockActualTime);
		}
	
		String stockActualPerson= Struts2Utils.getParameter("stockActualPerson");
		GsmEquipment gsmEquipment=null;
		for(String id:ids){
			gsmEquipment=gsmEquipmentDao.get(Long.valueOf(id));
			if("P".equals(type)){
				gsmEquipment.setStockPlanTime(plan);
				gsmEquipment.setStockPlanPerson(stockPlanPerson);
				gsmEquipment.setStockState("P");
			}else{
				gsmEquipment.setStockActualTime(actual);
				gsmEquipment.setStockActualPerson(stockActualPerson);
				gsmEquipment.setStockState("D");
			}
			gsmEquipmentDao.save(gsmEquipment);
		}
		
	}
	public void sendDisableMail(GsmEquipment gsmEquipment){
		String emailContent = "管理编号:"+gsmEquipment.getManagerAssets()+"设备名称:"+gsmEquipment.getEquipmentName()+"已停用禁止使用.";
		
		String dutyMan=gsmEquipment.getDutyLoginMan().trim();//责任人
		//String director = gsmEquipment.getDirectorLogin().trim();//主管
		String copyLoginMan = gsmEquipment.getCopyLoginMan();//抄送人
		if(dutyMan!=null && !"".equals(dutyMan)){
			User dutyManUser=ApiFactory.getAcsService().getUserByLoginNameAndBranchId(dutyMan,null);
			if(dutyManUser.getEmail()!=null && !"".equals(dutyManUser.getEmail())){
				AsyncMailUtils.sendMail(dutyManUser.getEmail(),"仪器变更通知!",emailContent.toString());
			}
		}
		/*if(director!=null && !"".equals(director)){
			User directorUser=ApiFactory.getAcsService().getUserByLoginNameAndBranchId(director,null);
			if(directorUser.getEmail()!=null && !"".equals(directorUser.getEmail())){
				AsyncMailUtils.sendMail(directorUser.getEmail(),"仪器变更通知!",emailContent.toString());
			}
		}*/
		if(copyLoginMan!=null&&!"".equals(copyLoginMan)){
			String a[]=copyLoginMan.split(",");
			for (int i = 0; i < a.length; i++) {
				String userId=a[i].toString();
				String email = ApiFactory.getAcsService().getUserByLoginName(userId).getEmail();
				if(StringUtils.isNotEmpty(email)){
					AsyncMailUtils.sendMail(email,"仪器变更通知!",emailContent.toString());
				}
			}
		}
