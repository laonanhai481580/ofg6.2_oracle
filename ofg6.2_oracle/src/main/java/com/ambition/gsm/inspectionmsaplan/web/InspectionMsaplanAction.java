...
public class InspectionMsaplanAction extends CrudActionSupport<InspectionMsaplan> {
	...
	protected void prepareModel() throws Exception {
		if (id == null) {
			...
			inspectionMsaplan.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
...
//		Integer weight =user.getWeight();
		try { 
			if("TP".equals(companyName)){
				page = inspectionMsaplanManager.listState(page, type,subName);
			}else{
				page = inspectionMsaplanManager.listState(page,type,null);
			}
			this.renderText(PageUtils.pageToJson(page));
		} catch (Exception e) {
			logger.error("MSA校验检索数据失败：", e);
