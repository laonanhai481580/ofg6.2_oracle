...
public class GsmForeignReportDao extends HibernateDao<GsmForeignReport,Long>{
	public Page<GsmForeignReport> list(Page<GsmForeignReport> page){
		return findPage(page,"from GsmForeignReport t where t.companyId=?",ContextUtils.getCompanyId());
	}
	...
	public Page<GsmForeignReport> search(Page<GsmForeignReport> page){
		return searchPageByHql(page,"from GsmForeignReport t where t.companyId=?", ContextUtils.getCompanyId());
	}
	public GsmForeignReport listPlan(String pId){
		String hql = "from GsmForeignReport t where t.planId=? and t.companyId=?";
		@SuppressWarnings("unchecked")
		List<GsmForeignReport> list = createQuery(hql,pId,ContextUtils.getCompanyId()).list();
		if(list.size()>0){
			return list.get(0);
		}
		return null;
