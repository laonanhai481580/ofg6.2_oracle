...
public class InspectionWarnUserDao extends HibernateDao<InspectionWarnUser, Long> {
	...
	public Page<InspectionWarnUser> list(Page<InspectionWarnUser> page){
		return findPage(page, "from InspectionWarnUser t where t.companyId = ?",ContextUtils.getCompanyId());
	}
	
	public List<InspectionWarnUser> getAllInspectionWarnUser(){
		return find("from InspectionWarnUser t where t.companyId = ?",ContextUtils.getCompanyId());
	}

    public Page<InspectionWarnUser> search(Page<InspectionWarnUser> page) {
        return searchPageByHql(page, "from InspectionWarnUser t where t.companyId = ?",ContextUtils.getCompanyId());
    }
}
