				...
				if(gsmForeignReport.getCheckDate().getTime()<=inspectionPlan.getInspectionPlanDate().getTime()){
				 ...
				 inspectionPlanManager.saveInspectionPlan(inspectionPlan);
				  ...
				  Calendar c = Calendar.getInstance();//获得一个日历的实例
			       ...
			       gsmForeignReport.setIsAchieve("Y");
			       gsmForeignReportDao.save(gsmForeignReport);
				//生成下一条计划
/*				  if(gsmForeignReport.getHasNext()==null||!gsmForeignReport.getHasNext().equals("是")){
			...
			throw new RuntimeException("请填写校验日期,校验结果!");
		}
	}
	public Page<GsmForeignReport> listState(Page<GsmForeignReport> page,String state ,String str){
		String hql = " from GsmForeignReport e where e.gsmState=? and e.companyId=?";
...
//		}
		...
		return gsmForeignReportDao.searchPageByHql(page, hql, searchParams.toArray());
	}
}
