...
public class InspectionPlanManager {
	...
	public Page<InspectionPlan> getPage(Page<InspectionPlan>page ,String inspectionState,String operator,String str){
		return inspectionPlanDao.getPage(page, inspectionState, operator,str);
	}
	...
	public void test(String ids,String bd) throws Exception {
		for(String id : ids.split(",")){
			...
			if("N".equals(bd)){
				...
				gsmInnerCheckReport.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
				...
				gsmInnerCheckReport.setGsmState(inspectionPlan.getGsmState());//隐藏状态
				gsmInnerCheckReport.setProcessSection(inspectionPlan.getProcessSection());
				gsmInnerCheckReport.setFactoryClassify(inspectionPlan.getFactoryClassify());//厂区
				gsmInnerCheckReport.setBusinessUnits(inspectionPlan.getBusinessUnits());
				gsmInnerCheckReport.setTaskProgress("待提交");
...
//				gsmForeignReport.setFormNo(formCodeGenerated.generateGsmEntrustNo());
				...
				gsmForeignReport.setProcessSection(inspectionPlan.getProcessSection());
				gsmForeignReport.setBusinessUnits(inspectionPlan.getBusinessUnits());
				gsmForeignReport.setFactoryClassify(inspectionPlan.getFactoryClassify());
				gsmForeignReport.setIsAchieve("N");
...
//			gsmEquipment.setMeasurementState(GsmEquipment.STATE_CHECK);
			...
			if("内校".equals(bd)){
				...
				gsmInnerCheckReport.setCreatedTime(new Date());
				gsmInnerCheckReport.setCreator("system");
				gsmInnerCheckReport.setModifiedTime(new Date());
				gsmInnerCheckReport.setModifier("system");
				gsmInnerCheckReport.setModifierName("system");
			...
			if("外校".equals(bd)){
				...
				gsmForeignReport.setCreatedTime(new Date());
				gsmForeignReport.setCreator("system");
				gsmForeignReport.setModifiedTime(new Date());
				gsmForeignReport.setModifier("system");
				gsmForeignReport.setModifierName("system");
	...
	public Page<InspectionPlan> listState(Page<InspectionPlan> page,String state,String str){
		
		String hql = " from InspectionPlan e where e.gsmState=? and (e.inspectionState=? or e.inspectionState=?) ";
		List<Object> searchParams = new ArrayList<Object>();
		searchParams.add(state);
		searchParams.add(InspectionPlan.STATE_CHECk_WAIT);
		searchParams.add(InspectionPlan.STATE_CHECk_IN);
//		if("TP南昌1号园区".equals(str)){
//			hql=hql+" and e.businessUnits=?";
//			searchParams.add("南昌园区");
//		}else if("TP苏州厂区".equals(str)){
//			hql=hql+" and e.businessUnits=?";
//			searchParams.add("苏州园区");
//		}else if("TP深圳厂区".equals(str)){
//			hql=hql+" and e.businessUnits=?";
...
//		}
		...
		return inspectionPlanDao.searchPageByHql(page, hql, searchParams.toArray());
