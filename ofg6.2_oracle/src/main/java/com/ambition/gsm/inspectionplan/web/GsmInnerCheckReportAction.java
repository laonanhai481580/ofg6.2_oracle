...
//		String processSection = Struts2Utils.getParameter("processSection");
		...
		try{
			if("TP".equals(companyName)){
				if(weight==3){
					page = gsmInnerCheckReportManager.listState(page, type,null);
				}else{
					page = gsmInnerCheckReportManager.listState(page, type,subName);
				}
			}else{
				page = gsmInnerCheckReportManager.listState(page, type,null);
			}
	...
	public String delete() throws Exception {
		// TODO Auto-generated method stub
		try {
			String str=gsmInnerCheckReportManager.deleteEntity(deleteIds);
