...
public class MaintainDao extends HibernateDao<Maintain,Long>{
	public Page<Maintain> list(Page<Maintain> page){
		return findPage(page,"from Maintain m where m.companyId=?",ContextUtils.getCompanyId());
	}
	...
	public Page<Maintain> search(Page<Maintain> page){
		return searchPageByHql(page,"from Maintain m where m.companyId=?",ContextUtils.getCompanyId());
	}
} 
