...
public class CheckItem extends IdEntity{
	...
	private String sampleSchemeType;//抽样方案类型,正常,加严,免检
	...
	private String parentName;//项目父类名称 用于前台显示
	private Integer parentRowSpan;//项目父类合并数量
	@Column(length=1000)
	...
	public void setIiar(IncomingInspectionActionsReport iiar) {
		...
		if(iiar == null){
			...
			inspectionDate = null;
			requestCheckNo = null;
			setBusinessUnitCode(null);
		}else{
			supplierCode = iiar.getSupplierCode();
			supplierName = iiar.getSupplierName();
			checkBomCode = iiar.getCheckBomCode();
			checkBomName = iiar.getCheckBomName();
			inspectionDate = iiar.getInspectionDate();
			requestCheckNo = iiar.getRequestCheckNo();
			setBusinessUnitCode(iiar.getBusinessUnitCode());
		}
	}
	...
	public String getSchemeStartDateStr() {
		return DateUtil.formateTimeStr(schemeStartDate);
	}
