...
/*		 String businessUnitName = CommonUtil.getDataByLogin();
	...
	/**
	  ...
	  * @return
	 */
	public InspectingIndicator getInspectingIndicator(String materialCode,String businessUnitName){
		String hql = "from InspectingIndicator i where i.materielCode = ?";
		if(businessUnitName!=null){
			hql+=" and i.businessUnitName='"+businessUnitName+"' ";
		}
	...
	public String deleteInspectingIndicator(String deleteIds){
		...
		for(String id : ids){
			...
			if(inspectingIndicator.getId() != null){
				...
				sb.append(inspectingIndicator.getMaterielCode() + ",");
				//保存更记录记录,修改标准的最新标志
	...
	public List<InspectingIndicator> getAllInspectingIndicators(){
		return inspectingIndicatorDao.getAllInspectingIndicators();
	}
	// 封装不良细项结果数据集的JSON格式
