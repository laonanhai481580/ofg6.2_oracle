...
public class ItemIndicatorManager {
	 ...
	 * @return
	 */
	public JSONObject copyInspectingIndicators(){
		String copyId=Struts2Utils.getParameter("id");
		String materielCode=Struts2Utils.getParameter("materielCode");
		String materielName=Struts2Utils.getParameter("materielName");
		String materielType=Struts2Utils.getParameter("materielType");
		String figureNumber=Struts2Utils.getParameter("figureNumber");
		InspectingIndicator inspectingIndicator = inspectingIndicatorDao.get(Long.valueOf(copyId));
		JSONObject obj = new JSONObject();
			if(StringUtils.isNotEmpty(copyId)){
				if(!isExistInspectingIndicator(null,materielCode,null)){
					...
					targetInspectingIndicator.setMaterielCode(materielCode);
					targetInspectingIndicator.setMaterielName(materielName);
					targetInspectingIndicator.setMaterielType(materielType);
					targetInspectingIndicator.setFigureNumber(figureNumber);
					targetInspectingIndicator.setBaseType(inspectingIndicator.getBaseType());
					targetInspectingIndicator.setCompanyId(ContextUtils.getCompanyId());
					targetInspectingIndicator.setCreatedTime(new Date());
					targetInspectingIndicator.setCreator(ContextUtils.getLoginName());
					...
					if(businessUnitName!=null){
						targetInspectingIndicator.setBusinessUnitName(businessUnitName);
					}else{
						targetInspectingIndicator.setBusinessUnitName(inspectingIndicator.getBusinessUnitName());
					}
					...
					for(ItemIndicator itemIndicator : inspectingIndicator.getItemIndicators()){
						...
						targetIndicator.setAllowProportion(itemIndicator.getAllowProportion());
						targetIndicator.setIndicatorUnit(itemIndicator.getIndicatorUnit());
						targetIndicator.setIndicatorMethod(itemIndicator.getIndicatorMethod());
						...
						targetIndicator.setBusinessUnitName(itemIndicator.getBusinessUnitName());
						itemIndicatorDao.save(targetIndicator);
					}
				}else{
					obj.put("error",true);
					obj.put("message","该物料编码已存在检验标准！");
				}
			}		
			obj.put("error",false);
			obj.put("message","复制成功！");
		return obj;
	}
...
//			}
	 ...
	 * @return
	 */
	private Boolean isExistInspectingIndicator(Long id,String code,String businessUnitName){
	...
	public String importIndicator(File file,String fileName) throws Exception{
		...
		try {
			...
			for(int k=0;k<totalSheets;k++){
			    ...
			    if(materielNameCell != null){
			    	materielName = (ExcelUtil.getCellValue(materielNameCell)+"").trim();
			    }
			    if(materielName==null){
			    	throw new  AmbFrameException("Sheet"+k+"的物料名称值不能为空");
			    }
			    //厂区
			    Cell changquCell = ExcelUtil.getNextCell(sheet,cellMap.get("厂区"), cellRangeAddressMap);
			    String changqu = null;
			    if(changquCell != null){
			    	changqu = (ExcelUtil.getCellValue(changquCell)+"").trim();
			    }			    
			    //物料类别
			    ...
			    Cell sipCodeCell = ExcelUtil.getNextCell(sheet,cellMap.get("SIP编号"), cellRangeAddressMap);
			    String sipCode = null;
			    if(sipCodeCell != null){
			    	sipCode = (ExcelUtil.getCellValue(sipCodeCell)+"").trim();
			    }
			    Cell sipVersionCell = ExcelUtil.getNextCell(sheet,cellMap.get("SIP版本"), cellRangeAddressMap);
			    String sipVersion = null;
			    if(sipVersionCell != null){
			    	sipVersion = (ExcelUtil.getCellValue(sipVersionCell)+"").trim();
			    }
			    Cell baseTypeCell = ExcelUtil.getNextCell(sheet,cellMap.get("抽样方案"), cellRangeAddressMap);
			    String  baseType= null;
			    if(baseTypeCell != null){
			    	baseType = (ExcelUtil.getCellValue(baseTypeCell)+"").trim();
			    }
			    if(baseType==null){
			    	baseType = SampleCodeLetter.GB_TYPE;
			    }else if(baseType.equals("NA")){
			    	baseType="NA";
			    }else{
			    	boolean flag=false;
			    	List<Option> baseTypeOptions = ApiFactory.getSettingService().getOptionsByGroupCode("iqc-sampling-base-type");
					for(Option option : baseTypeOptions){
						if(option.getValue().equals(baseType)){
							flag=true;
						};
					}
					if(!flag){
						throw new  AmbFrameException("Sheet"+k+"的抽样方案值不符合标准！");	
					}
			    }			    
				List<Option> options = sampleCodeLetterManager.getInspectionLevelOptions(baseType);
				Map<String,String> levelMap = new HashMap<String, String>();
				Map<String,String> levelMap1 = new HashMap<String, String>();
				...
				for(Option option : options){
					...
					if(!baseType.equals(SampleCodeLetter.MIL_TYPE)){
						String name1 = name.substring(2);
						levelMap1.put(name1,option.getValue());
					}					
				...
				String businessUnitName = CommonUtil.getDataByLogin();
				if(changqu!=null){
					businessUnitName=changqu;
				}
			    if(isExistInspectingIndicator(null,materielCode, businessUnitName)){
					throw new AmbFrameException("SHEET"+k+"【"+sheet.getSheetName()+"】中,"+businessUnitName+"编码【"+materielCode+"】的检验标准已经存在,不能导入！");
				}
				InspectingIndicator inspectingIndicator = new InspectingIndicator();
				...
				inspectingIndicator.setSipCode(sipCode);
				inspectingIndicator.setSipVersion(sipVersion);
				inspectingIndicator.setBaseType(baseType);
				String key = figureNumber + materielCode;
				productNameMap.put(key, inspectingIndicator);
				if(businessUnitName!=null){
				    inspectingIndicator.setBusinessUnitName(businessUnitName);
				}
				//先保存检验标准文件,保存对应的ID
				...
				while(rows.hasNext()){
					...
					itemName = CommonUtil.replaceSymbols(itemName);
				    ...
				    tempCell = cellMap.get("检测设备编号");
				    if(tempCell!=null){
				    	cell = row.getCell(tempCell.getColumnIndex());
				    	if(cell != null){
				    		equipmentNumber = (ExcelUtil.getCellValue(cell)+"");
						}
					}				    				    
					...
					if(cell != null){
						...
						if(inspectionLevel.indexOf("Ⅰ")>-1){
							inspectionLevel=inspectionLevel.replace("Ⅰ", "I");
						}
						if(inspectionLevel.indexOf("Ⅱ")>-1){
							inspectionLevel=inspectionLevel.replace("Ⅱ", "II");
						}
						if(inspectionLevel.indexOf("Ⅲ")>-1){
							inspectionLevel=inspectionLevel.replace("Ⅲ", "III");
						}
						if(inspectionLevel.indexOf("Ⅳ")>-1){
							inspectionLevel=inspectionLevel.replace("Ⅳ", "IV");
						}
						if(inspectionLevel.indexOf("Ⅴ")>-1){
							inspectionLevel=inspectionLevel.replace("Ⅴ", "V");
						}
						if(inspectionLevel.indexOf("Ⅵ")>-1){
							inspectionLevel=inspectionLevel.replace("Ⅵ", "VI");
						}
						if(inspectionLevel.indexOf("Ⅶ")>-1){
							inspectionLevel=inspectionLevel.replace("Ⅶ", "VII");
						}
						if(StringUtils.isNotEmpty(inspectionLevel)&&!isGaoYi){
							...
							}else if(levelMap1!=null&&levelMap1.containsKey(inspectionLevel)){
								inspectionLevel = levelMap1.get(inspectionLevel);
							}
						}
					}
					...
					if(cell != null){
						...
						if(baseType.equals("NA")){
							throw new AmbFrameException("单片检验的物料检验数量不能为空!");
						}
					}
					...
					if(cell != null){
						...
						standards = CommonUtil.replaceSymbols(standards);
					}
					//值类型
					...
					if(cell != null){
						...
						if(StringUtils.isNotEmpty(value)){
							//是角度值时
							if(CommonUtil.isDegree(value)){
								valueType = ItemIndicator.VALUETYPE_DEGREE;
								levela = CommonUtil.degreeStrToNumber(value);
							}else{
					...
					if(cell != null){
						...
						if(StringUtils.isNotEmpty(value)){
							...
							if(CommonUtil.isDegree(value)){
								valueType = ItemIndicator.VALUETYPE_DEGREE;
								levelb = CommonUtil.degreeStrToNumber(value);
							}else{
								if(!CommonUtil.isDouble(value) && !value.substring(0,1).equals("-")){
					...
					Double allowProportion = null;
					tempCell = cellMap.get("允收比例%");
					if(tempCell!=null){
						cell = row.getCell(tempCell.getColumnIndex());
						if(cell != null){
							String value = (ExcelUtil.getCellValue(cell)+"").trim();
							if(StringUtils.isNotEmpty(value)){
								if(CommonUtil.isDegree(value)){
									valueType = ItemIndicator.VALUETYPE_DEGREE;
									allowProportion = CommonUtil.degreeStrToNumber(value);
								}else{
									if(!CommonUtil.isDouble(value) && !value.substring(0,1).equals("-")){
										throw new AmbFrameException("SHEET"+k+"物料【"+materielName+"】中允收比例【"+value+"】不是有效数字!");
									}
									allowProportion = Double.valueOf(df.format(Double.valueOf(value)));
								}
							}
						}
					}										
					...
					itemIndicator.setAllowProportion(allowProportion);
					itemIndicator.setEquipmentNumber(equipmentNumber);
					if(SampleCodeLetter.MIL_TYPE.equals(baseType)){
...
//				inspectingIndicator.setMaterielName(materielName);
	...
	public static boolean isFirstGranterSecond(String[] firstStrs,String[] secondStrs,int index){
		if(index>=firstStrs.length
			...
			if(CommonUtil.isInteger(firstStrs[index])
					&&CommonUtil.isInteger(secondStrs[index])){
					...
					if(InspectingItem.COUNTTYPE_METERING.equals(checkItem.getCountType())){
						...
						if(valueList.size()>0){
							...
							if(qualityFeature != null){
								...
								List<String> list=spcImportManager.backImportValues(qualityFeature.getId(),incomingInspectionActionsReport.getInspectionDate(),null,null,hisIds,valueList, null);
