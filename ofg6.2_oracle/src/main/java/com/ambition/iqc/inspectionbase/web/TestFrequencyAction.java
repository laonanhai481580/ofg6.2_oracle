...
public class TestFrequencyAction extends BaseAction<TestFrequency> {
	...
	protected void prepareModel() throws Exception {
		if(id==null){
			...
			testFrequency.setIsErp("否");
			testFrequency.setIsMaterial("否");
			testFrequency.setIsOrt("否");
			testFrequency.setIsHsf("否");
			testFrequency.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
		}else {
	...
	public String saveOld() throws Exception {
		...
		for(Object obj : list){			
			...
			if(objs[4]!=null){
				businessUnitName=objs[4].toString();
			}
			if(businessUnitName!=null&&businessUnitName.equals("公用")){
				List<Option> processSection = ApiFactory.getSettingService().getOptionsByGroupCode("processSections");
				for (Option option : processSection) {
					if(supplierCode!=null&&checkBomCode!=null&&!testFrequencyManager.isExistTestFrequency(null, supplierCode,checkBomCode,option.getName())){
						...
						testFrequency.setLastModifiedTime(new Date());
						testFrequency.setLastModifier(ContextUtils.getUserName());
						testFrequency.setBusinessUnitName(option.getName());
				...
				if(supplierCode!=null&&checkBomCode!=null&&!testFrequencyManager.isExistTestFrequency(null, supplierCode,checkBomCode,businessUnitName)){
					TestFrequency testFrequency=new TestFrequency();
					testFrequency.setCreatedTime(new Date());
					testFrequency.setCompanyId(ContextUtils.getCompanyId());
					testFrequency.setCreator(ContextUtils.getUserName());
					testFrequency.setLastModifiedTime(new Date());
					testFrequency.setLastModifier(ContextUtils.getUserName());
					testFrequency.setBusinessUnitName(businessUnitName);
					testFrequency.setSupplierCode(supplierCode);
					testFrequency.setCheckBomCode(checkBomCode);
					testFrequency.setSupplierName(supplierName);
					testFrequency.setCheckBomName(checkBomName);
	...
	@Action("material-list-datas")
	public String materialListDatas() throws Exception {
		try {
			page = testFrequencyManager.searchMaterial(page);
			renderText(PageUtils.pageToJson(page));
			logUtilDao.debugLog("查询", "测试频率维护");
		} catch (Exception e) {
			log.error("查询测试频率维护信息失败  ",e);
...
/*	@Action("tes")
	   ...
	   public JSONObject test(){
		   JSONObject result = new JSONObject();
			...
			if(testFrequency!=null){				
				...
				String testUnit=testFrequency.getTestUnit();
				...
				if(ortTestCycle!=null&&isOrt!=null&&isOrt.equals("是")){
					...
					if(lastOrtTestDate==null){									
						...
						if(report!=null){
							EntrustOrt entrustOrt=entrustOrtManager.getEntity(Long.valueOf(report.getOrdReportId()));
							if(entrustOrt!=null){
								testFrequency.setLastOrtTestDate(entrustOrt.getConsignableDate());
								testFrequency.setLastOrtReportId(report.getOrdReportId());
								testFrequency.setLastOrtReportNo(report.getOrdReportNo());
								cal.setTime(entrustOrt.getConsignableDate());
							}
							if("天".equals(testUnit)){
								cal.add(Calendar.DATE,ortTestCycle);
							}else{
				...
				if(hsfTestCycle!=null&&isHsf!=null&&isHsf.equals("是")){
					...
					if(lastHsfTestDate==null){									
						...
						if(report!=null){
							EntrustHsf entrustHsf=entrustHsfManager.getEntity(Long.valueOf(report.getHisReportId()));
							if(entrustHsf!=null){
								testFrequency.setLastHsfTestDate(entrustHsf.getConsignableDate());
								testFrequency.setLastHsfReportId(report.getHisReportId());
								testFrequency.setLastHsfReportNo(report.getHisReportNo());
								cal.setTime(entrustHsf.getConsignableDate());
							}
							if("天".equals(testUnit)){
								cal.add(Calendar.DATE,hsfTestCycle);
							}else{
   ...
   public JSONObject testTP(){
	   JSONObject result = new JSONObject();
		Calendar cal = Calendar.getInstance();
		String supplierCode=Struts2Utils.getParameter("supplierCode");
		String checkBomCode=Struts2Utils.getParameter("checkBomCode");
		String businessUnitName = CommonUtil.getDataByLogin();
		TestFrequency testFrequency=testFrequencyManager.searchFrequencyTP(supplierCode, checkBomCode,businessUnitName);
		String materialType=null;
		if(testFrequency!=null){
			materialType=testFrequency.getMaterialType();
		}		
		Boolean flag=false;
		if(materialType!=null&&!"".equals(materialType)){
			testFrequency=testFrequencyManager.searchFrequencyMaterial(supplierCode,materialType ,businessUnitName);
			flag=true;
		}
		Date nowDate=new Date();			
		cal.setTime(nowDate);
		...
		if(testFrequency!=null){			
			...
			if(ortTestCycle!=null&&isOrt!=null&&isOrt.equals("是")){
				...
				result.put("ortEmpty", false);
				if(lastOrtTestDate==null){									
					IncomingInspectionActionsReport report=incomingInspectionActionsReportManager.searchReportTP(supplierCode, checkBomCode,"ort",businessUnitName);
					if(report!=null){						
						EntrustOrt entrustOrt=entrustOrtManager.getEntity(Long.valueOf(report.getOrdReportId()));
						if(entrustOrt!=null){
							testFrequency.setLastOrtTestDate(entrustOrt.getConsignableDate());
			...
			//周期为一个月
			Date  lastOrtTestDate1=testFrequency.getLastOrtTestDate1();
			String lastOrtReportId1=testFrequency.getLastOrtReportId1();
			String lastOrtReportNo1=testFrequency.getLastOrtReportNo1();
			if(lastOrtTestDate1==null){
				IncomingInspectionActionsReport report=incomingInspectionActionsReportManager.searchReportTPSpecial(supplierCode, checkBomCode,"one",businessUnitName);
				if(report!=null){						
					EntrustOrt entrustOrt=entrustOrtManager.getEntity(Long.valueOf(report.getOrdReportId1()));
					if(entrustOrt!=null){
						testFrequency.setLastOrtTestDate1(entrustOrt.getConsignableDate());
						testFrequency.setLastOrtReportId1(report.getOrdReportId1());
						testFrequency.setLastOrtReportNo1(report.getOrdReportNo1());
						cal.setTime(entrustOrt.getConsignableDate());
					}						
					...
					Long endDate=cal.getTimeInMillis();
					if(nowDateLong<endDate){
						result.put("ort1Need", false);
						result.put("ort1First", false);
						result.put("lastOrtReportId1", report.getOrdReportId1());
						result.put("lastOrtReportNo1", report.getOrdReportNo1());
					}else{
						result.put("ort1Need", true);
						result.put("ort1First", false);
					}
				}
			}else{
				cal.setTime(lastOrtTestDate1);
				cal.add(Calendar.MONTH,1);
				Long endDate=cal.getTimeInMillis();
				if(nowDateLong<endDate){
					result.put("ort1Need", false);
					result.put("ort1First", false);
					result.put("lastOrtReportId1", lastOrtReportId1);
					result.put("lastOrtReportNo1", lastOrtReportNo1);
				}else{
					result.put("ort1Need", true);
					result.put("ort1First", false);
				}
			...
			//周期为三个月
			Date  lastOrtTestDate3=testFrequency.getLastOrtTestDate3();
			String lastOrtReportId3=testFrequency.getLastOrtReportId3();
			String lastOrtReportNo3=testFrequency.getLastOrtReportNo3();
			if(lastOrtTestDate3==null){
				IncomingInspectionActionsReport report=incomingInspectionActionsReportManager.searchReportTPSpecial(supplierCode, checkBomCode,"three",businessUnitName);
				if(report!=null){						
					EntrustOrt entrustOrt=entrustOrtManager.getEntity(Long.valueOf(report.getOrdReportId3()));
					if(entrustOrt!=null){
						testFrequency.setLastOrtTestDate3(entrustOrt.getConsignableDate());
						testFrequency.setLastOrtReportId3(report.getOrdReportId3());
						testFrequency.setLastOrtReportNo3(report.getOrdReportNo3());
						cal.setTime(entrustOrt.getConsignableDate());
					}						
					cal.add(Calendar.MONTH,3);
					Long endDate=cal.getTimeInMillis();
					if(nowDateLong<endDate){
						result.put("ort3Need", false);
						result.put("ort3First", false);
						result.put("lastOrtReportId3", report.getOrdReportId3());
						result.put("lastOrtReportNo3", report.getOrdReportNo3());
					}else{
						result.put("ort3Need", true);
						result.put("ort3First", false);
					}
				}
			}else{
				cal.setTime(lastOrtTestDate3);
				cal.add(Calendar.MONTH,3);
				Long endDate=cal.getTimeInMillis();
				if(nowDateLong<endDate){
					result.put("ort3Need", false);
					result.put("ort3First", false);
					result.put("lastOrtReportId3", lastOrtReportId3);
					result.put("lastOrtReportNo3", lastOrtReportNo3);
				}else{
					result.put("ort3Need", true);
					result.put("ort3First", false);
				}
			...
			if(hsfTestCycle!=null&&isHsf!=null&&isHsf.equals("是")){
				result.put("hsfEmpty", false);
				if(lastHsfTestDate==null){									
					IncomingInspectionActionsReport report=incomingInspectionActionsReportManager.searchReportTP(supplierCode, checkBomCode,"hsf",businessUnitName);
					if(report!=null){
						EntrustHsf entrustHsf=entrustHsfManager.getEntity(Long.valueOf(report.getHisReportId()));
						if(entrustHsf!=null){
							testFrequency.setLastHsfTestDate(entrustHsf.getConsignableDate());
			...
			if(flag){
				result.put("testFrequency", false);
				result.put("isMaterial", true);
			}else{
				result.put("testFrequency", false);
				result.put("isMaterial", false);
			}
	...
	public String isErp() throws Exception {
		JSONObject obj = new JSONObject();
		id=Long.valueOf(Struts2Utils.getParameter("id"));
		testFrequency=testFrequencyManager.getTestFrequency(id);
		if("是".equals(testFrequency.getIsErp())){
			obj.put("isErp",true);
		}else{
			obj.put("isErp",false);
		}
