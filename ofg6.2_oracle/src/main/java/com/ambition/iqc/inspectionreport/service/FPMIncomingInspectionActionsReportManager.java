...
@Service
@Transactional
public class FPMIncomingInspectionActionsReportManager {
...
//		return incomingInspectionActionsReport;
	 ...
	 */
	public Map<String,Object> getCheckItems(String businessUnitCode,String supplierCode,String checkBomCode,Double stockAmount,Date inspectionDate,List<JSONObject> existCheckItems) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		return getCheckItems(businessUnitCode,supplierCode, checkBomCode, stockAmount, inspectionDate, existCheckItems,ContextUtils.getCompanyId());
	}
	/**
	 * 根据供应商,物料编码,检验数查询检验项目
	 * @param supplierId
	...
	public Map<String,Object> getCheckItems(String businessUnitCode,String supplierCode,String checkBomCode,Double stockAmount,Date inspectionDate,List<JSONObject> existCheckItems,Long companyId) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		...
		SampleTransferRecord sampleTransferRecord = sampleTransferRecordManager.getSampleTransferRecord(businessUnitCode,supplierCode, checkBomCode,inspectionDate,companyId);
		if(sampleTransferRecord==null||!SampleScheme.EXEMPTION_TYPE.equals(sampleTransferRecord.getTargetRule())){
			...
			for(InspectingItem parentInspectingItem : parentInspectingItems){
				...
				for(ItemIndicator itemIndicator : indicators){
					...
					String aqls[]= null;
					if(SampleCodeLetter.MIL1051_TYPE.equals(baseType)||SampleCodeLetter.C0_TYPE.equals(baseType)){
					...
					if(itemIndicator.getInspectionAmount() != null){
						...
						}else if(SampleCodeLetter.MIL1051_TYPE.equals(baseType)||SampleCodeLetter.C0_TYPE.equals(baseType)){
							checkItem.setAql(aql);
...
//					checkItem.setInspectionMan(ContextUtils.getUserName());
				...
				if("计量".equals(checkItem.getCountType())){
					String result="";
					for (int j = 1; j < 15; j++) {						
						String fieldName="result"+j;
					...
					setProperty(checkItem, "resultsTotal", result);
				}
...
//			String message = incomingInspectionService.testuUpdateRequestCheckState(shuliang);
	...
	private void checkSampleTransitionRule(IncomingInspectionActionsReport incomingInspectionActionsReport){
		...
		//首先检查供应商,物料还有没有没有审核的
		String hql = "select count(s.id) from SampleTransferRecord s where s.companyId = ? and s.supplierCode = ? and s.checkBomCode = ? and s.auditState = ?";
		List<?> list = incomingInspectionActionsReportDao.find(hql,ContextUtils.getCompanyId(),incomingInspectionActionsReport.getSupplierCode(),incomingInspectionActionsReport.getCheckBomCode(),SampleTransferRecord.AUDITSTATE_NORMAL);
		if(Integer.valueOf(list.get(0).toString())>0){
			return;
		}
		...
		if("OK".equals(incomingInspectionActionsReport.getInspectionConclusion())){
			...
			if(sampleTransitionRule != null){
				...
				params.add(incomingInspectionActionsReport.getBusinessUnitCode());
				...
				hql = "select i.inspectionConclusion from IncomingInspectionActionsReport i where i.businessUnitCode = ? and i.supplierCode = ? and i.checkBomCode = ? and i.sampleSchemeType != ? ";
				...
				if(total >= sampleTransitionRule.getAmount()){
					...
					sampleTransferRecord.setBusinessUnitName(incomingInspectionActionsReport.getBusinessUnitName());
			...
			if(sampleTransitionRule != null){
				...
				params.add(incomingInspectionActionsReport.getSupplierCode());
				params.add(incomingInspectionActionsReport.getCheckBomCode());
				hql = "select i.inspectionConclusion from IncomingInspectionActionsReport i where i.businessUnitCode = ? and i.supplierCode = ? and i.checkBomCode = ? ";
				if(incomingInspectionActionsReport.getSchemeStartDate()!=null){
					hql += " and i.inspectionDate > ? ";
					params.add(incomingInspectionActionsReport.getSchemeStartDate());
				}
	...
	public void auditIncomingInspectionActionsReport(Long id,String auditText,String lastAuditText,
		...
		if(report != null){
			...
			report.setProcessingResult(auditText);
...
//	}
	...
	public Page<IncomingInspectionActionsReport> listUnCheck(Page<IncomingInspectionActionsReport>page,String checkItem) throws Exception{								
	...
	public Page<IncomingInspectionActionsReport> listWaitAuditSingle(Page<IncomingInspectionActionsReport>page,String checkItem){
		return incomingInspectionActionsReportDao.listAuditSingleFpm(page,checkItem);		
	}
...
//		}
	...
	public List<IncomingInspectionActionsReport> listAllForm(){
		return incomingInspectionActionsReportDao.listAllForm();
	}
...
//		splitReport.setInspectionNo(formCodeGenerated.generateIncomingInspectionReportCode(report.getCompanyId()));
	...
	public void exportReport(IncomingInspectionActionsReport incomingInspectionActionsReport) throws IOException{
		InputStream inputStream = null;
		try {
			/*String str="inspectionType,parentItemName,parentName,checkItemName,countType,checkMethod,inspectionAmount,specifications,unit,maxlimit,minlimit,equipmentNumber"
					+",allowProportion,results,standardValue,minResult,maxResult,avgResult";
			...
			List<CheckItem1> checkItems1=new ArrayList<CheckItem1>();
			List<CheckItem2> checkItems2=new ArrayList<CheckItem2>();
			for (CheckItem checkItem : checkItems) {
				if(checkItem.getInspectionType().indexOf("尺寸")!=-1){
					CheckItem1 checkItem1=new CheckItem1();
					CommonUtil.copyObj(checkItem, checkItem1, fieldNames);
					checkItem1.setIiar(incomingInspectionActionsReport);
					checkItems1.add(checkItem1);
				}else{
					CheckItem2 checkItem2=new CheckItem2();
					CommonUtil.copyObj(checkItem, checkItem2, fieldNames);
					checkItem2.setIiar(incomingInspectionActionsReport);
					checkItems2.add(checkItem2);
				}
			}
			incomingInspectionActionsReport.setCheckItems1(checkItems1);
			incomingInspectionActionsReport.setCheckItems2(checkItems2);*/
			IncomingInspectionActionsReport report = incomingInspectionActionsReport;
			inputStream = this.getClass().getClassLoader().getResourceAsStream("template/report/iqc-inspection-report-fpm.xls");
			...
			//检验记录格式化
			/*formatterMap.put("checkItems1.results",new ExportExcelFormatter() {
				public String format(Object value, int rowNum, String fieldName,Cell cell){
					String result = "";
					CheckItem1 item = (CheckItem1)value;
			...
			//检验记录格式化
			formatterMap.put("checkItems2.results",new ExportExcelFormatter() {
				public String format(Object value, int rowNum, String fieldName,Cell cell){
					String result = "";
					CheckItem2 item = (CheckItem2)value;
					if("计数".equals(item.getCountType())){
						return item.getResults();
					}else{
						Field[] fields = item.getClass().getDeclaredFields();
			...
			//检验记录格式化
			formatterMap.put("checkItems2.inspectionAmount",new ExportExcelFormatter() {
				public String format(Object value, int rowNum, String fieldName,Cell cell){
					String result = "n=";
					CheckItem2 item = (CheckItem2)value;
					if(item.getInspectionAmount()==null){
						...
						result+=item.getInspectionAmount().toString()+",Ac/Re=";
					}
					if(item.getAqlAc()==null){
						result+=" /=";
					}else{
						result+=item.getAqlAc().toString()+"/";
					}
					if(item.getAqlRe()==null){
						result+=" ";
					}else{
						result+=item.getAqlRe().toString()+"";
					}
					return result;
				}
			});
...
//			incomingInspectionService.updateRequestCheckState(report);
	...
	public List<String> serachCheckItems() {
		String hql="select i.checkItemName from CheckItem i  where i.inspectionType like '%尺寸%' and i.iiar.hiddenState='N'  and i.iiar.inspectionState='已完成'  group by i.checkItemName  ";
		Query query=null; 
		try {
			 query = incomingInspectionActionsReportDao.getSession().createQuery(hql.toString());
		} catch (Exception e) {
