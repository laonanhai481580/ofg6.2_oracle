...
import com.ambition.iqc.entity.CheckItem1;
import com.ambition.iqc.entity.CheckItem2;
...
import com.ambition.iqc.inspectionreport.dao.CheckItemDao;
...
//		return incomingInspectionActionsReport;
	 ...
	 */
	public Map<String,Object> getCheckItems(String processSection,String businessUnitCode,String supplierCode,String checkBomCode,Double stockAmount,Date inspectionDate,List<JSONObject> existCheckItems) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		return getCheckItems(processSection,businessUnitCode,supplierCode, checkBomCode, stockAmount, inspectionDate, existCheckItems,ContextUtils.getCompanyId());
	}
	/**
	 * 根据供应商,物料编码,检验数查询检验项目
	 * @param supplierId
	...
	public Map<String,Object> getCheckItems(String processSection,String businessUnitCode,String supplierCode,String checkBomCode,Double stockAmount,Date inspectionDate,List<JSONObject> existCheckItems,Long companyId) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
...
//			}
			hql = "from ItemIndicator i where i.companyId = ? and i.inspectingIndicator.materielCode = ? and i.inspectingIndicator.isMax=? and i.inspectingIndicator.businessUnitName = ?";
			itemIndicators = incomingInspectionActionsReportDao.find(hql,companyId,checkBomCode,true,processSection);
...
//				}
			...
			for(InspectingItem parentInspectingItem : parentInspectingItems){
				...
				for(ItemIndicator itemIndicator : indicators){
					...
					checkItem.setAllowProportion(itemIndicator.getAllowProportion());
					String level = itemIndicator.getInspectionLevel();
					String baseType=itemIndicator.getInspectingIndicator().getBaseType();
					if(baseType==null){
						baseType = sampleSchemeManager.getUseBaseType(companyId).getBaseType();
					}
					//1916标准特殊加严,从右到左
					...
					checkItem.setOrderNum(itemIndicator.getOrderNum());
					...
					checkItem.setEquipmentNumber(itemIndicator.getEquipmentNumber());
					...
					if(checkItem.getAllowProportion()!=null){
						Double allowProportion=checkItem.getAllowProportion();
						Integer inspectionAmount=checkItem.getInspectionAmount();
						Double a=inspectionAmount*allowProportion*0.01;
						Integer b=(int)Math.ceil(a);
						Integer c=(int)Math.floor(a);
						if(b==c){
							b++;
						}
						if(b>0){
							checkItem.setAqlRe(b);
						}
					}
	...
	public List<CheckItem> getCheckItems(IncomingInspectionActionsReport report,List<JSONObject> existCheckItems) throws NumberFormatException, IllegalAccessException, InvocationTargetException, NoSuchMethodException{
		...
		Map<String,Object> resultMap = getCheckItems(report.getProcessSection(),report.getBusinessUnitCode(),supplierCode, checkBomCode, stockAmount, inspectionDate, existCheckItems,report.getCompanyId());
	 ...
	 * @throws Exception 
	 */
	public void saveIncomingInspectionActionsReportTarget(IncomingInspectionActionsReport incomingInspectionActionsReport,List<JSONObject> checkItemArray) throws Exception{
		...
		CheckItem1 checkItems1=new CheckItem1();
		CheckItem2 checkItems2=new CheckItem2();
		List<CheckItem1> list1=new ArrayList<CheckItem1>();
		List<CheckItem2> list2=new ArrayList<CheckItem2>();
		if(incomingInspectionActionsReport.getId()!=null){
			incomingInspectionActionsReport.getCheckItems1().clear();
			incomingInspectionActionsReport.getCheckItems2().clear();
			incomingInspectionActionsReport.getCheckItems1().add(checkItems1);
			incomingInspectionActionsReport.getCheckItems2().add(checkItems2);
		}else{
			incomingInspectionActionsReport.setCheckItems1(list1);
			incomingInspectionActionsReport.setCheckItems2(list2);
			incomingInspectionActionsReport.getCheckItems1().add(checkItems1);
			incomingInspectionActionsReport.getCheckItems2().add(checkItems2);
		}
		incomingInspectionActionsReport.setSourceCopy("否");
	...
	public void saveIncomingInspectionActionsReport(IncomingInspectionActionsReport incomingInspectionActionsReport,List<JSONObject> checkItemArray) throws Exception{
		if(!(IncomingInspectionActionsReport.INPECTION_STATE_DEFAULT.equals(incomingInspectionActionsReport.getInspectionState())
				||IncomingInspectionActionsReport.INPECTION_STATE_RE_CHECK.equals(incomingInspectionActionsReport.getInspectionState())||IncomingInspectionActionsReport.INPECTION_STATE_SAVE_TEMP.equals(incomingInspectionActionsReport.getInspectionState()))){
...
//			}else{
				...
				incomingInspectionActionsReport.setCurrentAuditMan(incomingInspectionActionsReport.getAuditMan());
				incomingInspectionActionsReport.setCurrentAuditManLogin(incomingInspectionActionsReport.getAuditLoginMan());
//			}
		}else{
			incomingInspectionActionsReport.setInspectionState(IncomingInspectionActionsReport.INPECTION_STATE_SAVE_TEMP);
			incomingInspectionActionsReport.setCurrentAuditMan(null);
			incomingInspectionActionsReport.setCurrentAuditManLogin(null);
		}
...
//				saveErpResult(incomingInspectionActionsReport);
		...
		//同步数据到spc
		/*String allHisSpcSampleIds = incomingInspectionActionsReport.getSpcSampleIds()==null?"":incomingInspectionActionsReport.getSpcSampleIds();
		String allSpcSampleIds = ",";
			...
			for(JSONObject json : checkItemArray){
				...
				if("计量".equals(countType) && StringUtils.isNotEmpty(featureId)){
					if(StringUtils.isNotEmpty(values)){
						...
						if(feature != null&&feature.getSampleCapacity()!=null){
							String hisSampleIds = checkItem.getSpcSampleIds();
							if(StringUtils.isNotEmpty(hisSampleIds)){
								allHisSpcSampleIds = allHisSpcSampleIds.replaceAll("," + hisSampleIds,",");
							}
							spcSampleIds =insertSpcByResults(feature,incomingInspectionActionsReport.getInspectionDate(),hisSampleIds,values);
						}
						allSpcSampleIds += spcSampleIds;
					}
		...
		//删除不存在采集数据的子组(全局调用，涉及报告的修改，存在删除问题)
			String[] ids = allHisSpcSampleIds.split(",");
			for(String id : ids){
				id = id.trim();
			...
			//设置最新的采集数据ID
			incomingInspectionActionsReport.setSpcSampleIds(allSpcSampleIds);*/	
	}
...
//			String message = incomingInspectionService.testuUpdateRequestCheckState(shuliang);
	...
	public void auditIncomingInspectionActionsReport(Long id,String auditText,String lastAuditText,
		...
		if(report != null){
			...
			if(IncomingInspectionActionsReport.INPECTION_STATE_SUBMIT.equals(report.getInspectionState())&&report.getInspectionConclusion().equals("NG")&&report.getLastStateLoginMan()==null){
				throw new AmbFrameException("NG的单子必须选择上级审核人");
			}
			//调用接口回写检验结果
//			report.setApprovedBy(ContextUtils.getUserName());
//			report.setApprovedTime(new Date());
//			report.setProcessingBadQty(badQty);
//			report.setProcessingReceiveQty(receiveQty);
//			report.setProcessingResult(processingResult);
//			report.setCutRate(cutRate);
			String processingResult=Struts2Utils.getParameter("processingResult");
			String sqeMrbReportNo=Struts2Utils.getParameter("sqeMrbReportNo");
			String returnReportNo=Struts2Utils.getParameter("returnReportNo");
			...
			if(isLast!=null&&!"".equals(isLast)){
				...
				if(report.getInspectionConclusion().equals("OK")||report.getInspectionConclusion().equals("合格")){
					report.setInspectionState(IncomingInspectionActionsReport.INPECTION_STATE_AUDIT);
				}else{
					report.setInspectionState(IncomingInspectionActionsReport.INPECTION_STATE_EXCEPTION_CONFIRM);
				}				
				report.setCurrentAuditMan(null);
				report.setCurrentAuditManLogin(null);
				report.setProcessingResult(processingResult);
				report.setSqeMrbReportNo(sqeMrbReportNo);
				report.setReturnReportNo(returnReportNo);
			}else{
				...
				if(report.getInspectionConclusion().equals("OK")||report.getInspectionConclusion().equals("合格")){
					...
					report.setCurrentAuditMan(report.getLastStageMan());
					report.setCurrentAuditManLogin(report.getLastStateLoginMan());
					//邮件提醒SQE审核
					sendEmail(report);
				}								
...
//			saveErpResult(report);
	...
	public void sendEmail(IncomingInspectionActionsReport report){
		String warmingManLogin=report.getLastStateLoginMan();
		String message = "*最新预警信息:"+report.getInspectionDate()+",单号为【"+report.getInspectionNo()+"】的进料检验单判定NG，需要你审核，请及时处理！！" ;
		if(warmingManLogin!=null){
			String email = ApiFactory.getAcsService().getUserByLoginName(warmingManLogin).getEmail();
			if(StringUtils.isNotEmpty(email)){
				AsyncMailUtils.sendMail(email,"进料检验单NG待审核",message);
			}
		}		
	}	
	public String deleteIncomingInspectionActionsReport(String deleteIds){
		...
		for(String id: ids){
			...
			if(report != null){
				...
				report.setIsDelete("是");
				report.setDeleteDate(new Date());
				report.setDeleteMan(ContextUtils.getUserName());
				report.setDeleteManLogin(ContextUtils.getLoginName());
				sb.append(report.getInspectionNo() + ",");
			}
		}
		return sb.toString();
	}
	public IncomingInspectionActionsReport copyReport(IncomingInspectionActionsReport report){
		
		IncomingInspectionActionsReport targetRepor=new IncomingInspectionActionsReport();
		/*String str="batchNo,processSection,inspectionNo,checkBomCode,checkBomName,supplierCode,supplierName,lotNo,orderNo,erpInspectionNo,acceptNo,"
		+"enterDate,stockAmount,units,acknowledgeState,acknowledgeVersion,checkBomMaterialType,productStage,sampleSchemeType,isFactory,customerCode"
		+",packingFir,packingFirResult,packingText,packingResult,packingInspcetionMan,packingSec,packingSecResult,packingTre,packingTreResult"
		+",hisInspectionItem,hisPackingResult,hisReportNo,hisReportId,hisText,ordInspectionItem,ordPackingResult,ordReportNo,ordReportId"
		+",ordText,ordReportNo1,ordReportId1,ordReportNo3,ordReportId3,ordTextSpecial,inspectionDate,schemeStartDate,auditLoginMan,auditMan"
		+",lastStateLoginMan,lastStageMan,remark,appearanceAmount,appearanceUnAmount,appearanceAmountRate,wgBadRate,wgInspector,qualifiedAmount,functionAmount";*/
		try {
			CommonUtil.copyObj(report, targetRepor);
			targetRepor.setEntryId(null);
			targetRepor.setTargetCopy("是");
			targetRepor.setHiddenState("Y");
			targetRepor.setExceptionId(null);
			targetRepor.setExceptionNo(null);
			targetRepor.setProcessingResult(null);
			targetRepor.setSqeMrbReportNo(null);
			targetRepor.setReturnReportNo(null);
			targetRepor.setSqeReplyTime(null);
			targetRepor.setSqeCompleteTime(null);
			targetRepor.setLastStateText(null);
			targetRepor.setLastStateTime(null);
			targetRepor.setAuditText("同意");
			targetRepor.setSourceId(report.getId().toString());
			report.setSourceCopy("是");
			this.saveIncomingInspectionActionsReport(report);
	...
	public String deleteIncomingInspectionActionsReportSec(String deleteIds){
		String[] ids = deleteIds.split(",");
		StringBuilder sb = new StringBuilder("");
	...
	public void recycleClear(){
			List<IncomingInspectionActionsReport> list=incomingInspectionActionsReportDao.listRecycleAll();
			for (IncomingInspectionActionsReport incomingInspectionActionsReport : list) {
				if(incomingInspectionActionsReport != null){
					incomingInspectionActionsReportDao.delete(incomingInspectionActionsReport);
				}
			}
	}	
	public void exceptionConfirm(String deleteIds){
		String[] ids = deleteIds.split(",");
		for(String id: ids){
			IncomingInspectionActionsReport report = incomingInspectionActionsReportDao.get(Long.valueOf(id));
			if(report != null){
	...
	public void returnAudit(String deleteIds){
		String[] ids = deleteIds.split(",");
		for(String id: ids){
			IncomingInspectionActionsReport report = incomingInspectionActionsReportDao.get(Long.valueOf(id));
			if(report != null){
	...
	public String recycleBack(String deleteIds){
		String[] ids = deleteIds.split(",");
		StringBuilder sb = new StringBuilder("");
		for(String id: ids){
			IncomingInspectionActionsReport report = incomingInspectionActionsReportDao.get(Long.valueOf(id));
			if(report != null){
				report.setIsDelete("否");
				report.setDeleteDate(null);
				report.setDeleteMan(null);
				report.setDeleteManLogin(null);
				sb.append(report.getInspectionNo() + ",");
			}
		}
		return sb.toString();
...
//	}
	...
	public Page<IncomingInspectionActionsReport> listWaitAudit(Page<IncomingInspectionActionsReport>page,String checkItem){
		return incomingInspectionActionsReportDao.listAudit(page,checkItem);
	}
	public Page<IncomingInspectionActionsReport> listExceptionConfirm(Page<IncomingInspectionActionsReport>page,String checkItem){
		return incomingInspectionActionsReportDao.listExceptionConfirm(page,checkItem);		
	}
	public Page<IncomingInspectionActionsReport> listWaitAuditSingle(Page<IncomingInspectionActionsReport>page,String checkItem){
		return incomingInspectionActionsReportDao.listAuditSingle(page,checkItem);		
	}
	...
	public Page<IncomingInspectionActionsReport> listRecycle(Page<IncomingInspectionActionsReport>page){
		return incomingInspectionActionsReportDao.listRecycle(page);
	}
	...
	public Page<IncomingInspectionActionsReport> copylist(Page<IncomingInspectionActionsReport>page){
		return incomingInspectionActionsReportDao.copylist(page);
	}
	/**
	  * 方法名: 快速检索查询
	  * <p>功能说明：</p>
	...
	public List<IncomingInspectionActionsReport> listOutOrIn(){
		return incomingInspectionActionsReportDao.listOutOrIn();
	}
...
//			}
	...
	public Map<String,Object> getOneChartDatas(JSONObject params) throws Exception{
		...
		CommonUtil.buildHql(params, hql, searchParams,getFieldMap(),"i");
		//全部
		...
		List<Integer> data = new ArrayList<Integer>();
		while(CommonUtil.getYearMonthAndDate(startCal)<= CommonUtil.getYearMonthAndDate(endCal)){
			//横坐标天数的参数
			categories.add(startCal.get(Calendar.DAY_OF_MONTH));
			
			Map<String,Object> map = new HashMap<String, Object>();
			String dateStr = DateUtil.formateDateStr(startCal);
			Integer index = CommonUtil.getYearMonthAndDate(startCal);
	...
	public Map<String,Object> getOneChartDatasByMonths(JSONObject params) throws Exception{
		...
		while(CommonUtil.getYearAndMonth(startCal)<=CommonUtil.getYearAndMonth(endCal)){
			Integer startMonth = CommonUtil.getYearAndMonth(startCal);
			//横坐标月份的参数
...
//		splitReport.setInspectionNo(formCodeGenerated.generateIncomingInspectionReportCode(report.getCompanyId()));
		...
		report.setCurrentAuditMan(splitReport.getAuditMan());
		report.setCurrentAuditManLogin(splitReport.getAuditLoginMan());
		...
		try {
			...
			/*formatterMap.put("qualifiedRate",new ExportExcelFormatter() {
				public String format(Object value, int rowNum, String fieldName,Cell cell) {
					...
					return report.getQualifiedRate() + "%";
				}
			});*/
...
//			incomingInspectionService.updateRequestCheckState(report);
	...
	public String hiddenState(String hideId,String type){
		...
		for(String id : ids){
			...
			sb.append(incomingInspectionActionsReport.getInspectionNo()+ ",");
			incomingInspectionActionsReportDao.save(incomingInspectionActionsReport);
		}
		return sb.toString();
	}
	...
	public Map<String,Object> getSizeSpreadChartDatas(JSONObject params) throws Exception{
		params = convertJsonObject(params);
		String checkItem="";
		String processSection="";
		String checkBomCode="";
		if(params.containsKey("checkItem")){
			checkItem=params.get("checkItem").toString();
		}
		if(params.containsKey("processSection")){
			processSection=params.get("processSection").toString();
		}	
		if(params.containsKey("checkBomCode")){
			checkBomCode=params.get("checkBomCode").toString();
		}
		...
		//最大值
		Map<String,Object> series1 = new HashMap<String, Object>();
		series1.put("name", "最大值");
		List<Map<String,Object>> data1 = new ArrayList<Map<String,Object>>();
		
		//最小值
		Map<String,Object> series2 = new HashMap<String, Object>();
		series2.put("name", "最小值");
		List<Map<String,Object>> data2 = new ArrayList<Map<String,Object>>();
		
		//平均值
		Map<String,Object> series3 = new HashMap<String, Object>();
		series3.put("name", "平均值");
		List<Map<String,Object>> data3 = new ArrayList<Map<String,Object>>();		
		
		//规格值
		/*Map<String,Object> series4 = new HashMap<String, Object>();
		series4.put("name", "规格值");
		List<Map<String,Object>> data4 = new ArrayList<Map<String,Object>>();*/
		

	
		result.put("title", "尺寸分布趋势图");
		List<Integer> categories = new ArrayList<Integer>();
		result.put("categories", categories);
		result.put("yAxisTitle1","数<br/>据");
		result.put("max", 100);
		//查询进货报告子表中 最大值  最小值 平均值
		String hql="select i.minResult,i.maxResult,i.avgResult,i.standardValue,i.id from CheckItem i  where i.iiar.inspectionDate between ? and ?  ";
		List<Object> searchParams = new ArrayList<Object>();
		searchParams.add(startDate);
		searchParams.add(endDate);
		if(!checkItem.equals("")){
			hql=hql+" and i.checkItemName=? ";
			searchParams.add(checkItem);
		}
		if(!processSection.equals("")){
			hql=hql+" and i.iiar.processSection=? ";
			searchParams.add(processSection);
		}
		if(!checkBomCode.equals("")){
			hql=hql+" and i.iiar.checkBomCode=? ";
			searchParams.add(checkBomCode);
		}
		hql=hql+" and i.iiar.hiddenState='N'  and i.iiar.inspectionState=? and i.inspectionType like '%尺寸%' ";
		searchParams.add(IncomingInspectionActionsReport.INPECTION_STATE_AUDIT);
		Query query=null; 
		try {
			 query = incomingInspectionActionsReportDao.getSession().createQuery(hql.toString());
		} catch (Exception e) {
			e.printStackTrace();
		}
		for(int i=0;i<searchParams.size();i++){
		...
		for(Object obj : list){
			...
			//平均值
			String avgResult=null;
			if(objs[2]!=null){
				avgResult=objs[2].toString();
			}
			map=new HashMap<String, Object>();
			map.put("name","平均值");
			map.put("y", avgResult);
			...
			if(objs[3]!=null){
				standardValue=objs[3].toString();
			}
			map=new HashMap<String, Object>();
			map.put("name","规格值");
			map.put("y", standardValue);
			map.put("id", id);
			data4.add(map);*/
		}
		result.put("tableHeaderList", categories);
	...
	public List<String> serachCheckItems(String checkBomCode) {
		List<Object> searchParams = new ArrayList<Object>();		
		String hql="select i.checkItemName from CheckItem i  where i.inspectionType like '%尺寸%' and i.iiar.hiddenState='N'  and i.iiar.inspectionState='已完成' and i.iiar.checkBomCode=?  group by i.checkItemName  ";
		Query query=null; 
		try {
			 query = incomingInspectionActionsReportDao.getSession().createQuery(hql.toString());
		} catch (Exception e) {
	...
	public Page<CheckItem> listItems(Page<CheckItem>page,Long id){
		return checkItemDao.listItems(page, id);
	}
	 ...
	 * @return
	 */
	public IncomingInspectionActionsReport searchReportTP(String supplierCode,String checkBomCode,String type,String businessUnitName){
		String hql = "select i from IncomingInspectionActionsReport i where i.companyId = ? and i.supplierCode = ? and i.checkBomCode=?   ";
		List<Object> searchParams = new ArrayList<Object>();
		searchParams.add(ContextUtils.getCompanyId());
		searchParams.add(supplierCode);
	 ...
	 * @return
	 */
	public IncomingInspectionActionsReport searchReportTPSpecial(String supplierCode,String checkBomCode,String special,String businessUnitName){
		String hql = "select i from IncomingInspectionActionsReport i where i.companyId = ? and i.supplierCode = ? and i.checkBomCode=?   ";
		List<Object> searchParams = new ArrayList<Object>();
		searchParams.add(ContextUtils.getCompanyId());
		searchParams.add(supplierCode);
		...
		if(special.equals("one")){
			hql += "  and  i.ordReportId1 is not null ";
		}
		if(special.equals("three")){
			hql += "  and  i.ordReportId3 is not null ";
		}
		hql+="and i.productStage <> ? ";
		searchParams.add("样品");
		hql += " order by i.inspectionDate desc ";
	...
	public String exchange(String deleteIds, String type) {
		String[] ids = deleteIds.split(",");
		String inspectionNo="";
		for(String id: ids){
			if(id!=null){
				IncomingInspectionActionsReport incomingInspectionActionsReport=this.getIncomingInspectionActionsReport(Long.valueOf(id));				
				String sourceId=incomingInspectionActionsReport.getSourceId();
				IncomingInspectionActionsReport sourceReport=null;
				if(sourceId!=null&&!"".equals(sourceId)){
					 sourceReport=this.getIncomingInspectionActionsReport(Long.valueOf(sourceId));
				}
				if(type.equals("yes")){
					incomingInspectionActionsReport.setHiddenState("N");					
					if(sourceReport!=null){
						sourceReport.setHiddenState("Y");
					}
				}else{
					incomingInspectionActionsReport.setHiddenState("Y");					
					if(sourceReport!=null){
						sourceReport.setHiddenState("N");
					}
				}
				inspectionNo+=incomingInspectionActionsReport.getInspectionNo()+",";
				this.saveIncomingInspectionActionsReport(incomingInspectionActionsReport);
				this.saveIncomingInspectionActionsReport(sourceReport);
			}
		}
		return inspectionNo;
