...
public class SampleTransferRecordManager {
	...
	public SampleTransferRecord getSampleTransferRecord(String businessUnitName,String supplierCode,String checkBomCode,Date inspectionDate,Long companyId){
		...
		if("欧菲科技-CCM".equals(companyName)){
			hql = "from SampleTransferRecord s where s.companyId = ? and s.businessUnitName = ? and s.supplierCode = ? and s.checkBomMaterialType = ? and s.auditState = ? and s.auditTime < ? order by s.auditTime desc";
			Query query = sampleTransferRecordDao.createQuery(hql, companyId,businessUnitName,supplierCode,checkBomMaterialType,SampleTransferRecord.AUDITSTATE_PASS,inspectionDate);
			query.setMaxResults(1);
			@SuppressWarnings("unchecked")
			List<SampleTransferRecord> list = query.list();
			if(list.isEmpty()){
