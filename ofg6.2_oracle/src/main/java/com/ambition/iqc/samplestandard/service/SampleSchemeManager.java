...
import com.ambition.iqc.samplestandard.dao.AcceptanceQualityLimitDao;
...
public class SampleSchemeManager {
	...
	@Autowired
	private AcceptanceQualityLimitDao acceptanceQualityLimitDao;
	...
	public void sfdsd(){
		List<SampleScheme> list=querySampleSchemes("GB/T2828", "正常");
		for (SampleScheme sampleScheme : list) {
			SampleScheme sampleScheme2=new SampleScheme();
			sampleScheme2.setBaseType("MIL-STD-105E");
				sampleScheme2.setType(sampleScheme.getType());
				sampleScheme2.setAmount(sampleScheme.getAmount());
				sampleScheme2.setCountType(sampleScheme.getCountType());
				sampleScheme2.setCode(sampleScheme.getCode());
				sampleScheme2.setCompanyId(ContextUtils.getCompanyId());
				sampleScheme2.setCreatedTime(new Date());
				sampleScheme2.setCreator(ContextUtils.getLoginName());
				sampleScheme2.setLastModifiedTime(sampleScheme.getCreatedTime());
				sampleScheme2.setLastModifier(ContextUtils.getUserName());
				sampleSchemeDao.save(sampleScheme2);
				//复制项目
				for(AcceptanceQualityLimit acceptanceQualityLimit : sampleScheme.getAcceptanceQualityLimits()){
					AcceptanceQualityLimit acceptanceQualityLimit2 = new AcceptanceQualityLimit();
					acceptanceQualityLimit2.setCompanyId(ContextUtils.getCompanyId());
					acceptanceQualityLimit2.setCreatedTime(acceptanceQualityLimit.getCreatedTime());
					acceptanceQualityLimit2.setCreator(acceptanceQualityLimit.getCreator());
					acceptanceQualityLimit2.setLastModifiedTime(acceptanceQualityLimit.getCreatedTime());
					acceptanceQualityLimit2.setLastModifier(ContextUtils.getUserName());
					acceptanceQualityLimit2.setAc(acceptanceQualityLimit.getAc());
					acceptanceQualityLimit2.setCode(acceptanceQualityLimit.getCode());
					acceptanceQualityLimit2.setAmount(acceptanceQualityLimit.getAmount());
					acceptanceQualityLimit2.setType(acceptanceQualityLimit.getType());
					acceptanceQualityLimit2.setCountType(acceptanceQualityLimit.getCountType());
					acceptanceQualityLimit2.setBaseType("MIL-STD-105E");
					acceptanceQualityLimit2.setAql(acceptanceQualityLimit.getAql());
					acceptanceQualityLimit2.setRe(acceptanceQualityLimit.getRe());
					acceptanceQualityLimit2.setSampleScheme(sampleScheme2);
					acceptanceQualityLimitDao.save(acceptanceQualityLimit2);
				}
		}
	}	
	
	public List<SampleScheme> querySampleSchemes(String baseType,String type){
		String hql = "from SampleScheme s where s.companyId = ? and s.type = ? and s.baseType = ? ";
		return sampleSchemeDao.find(hql,ContextUtils.getCompanyId(),type,baseType);
	}
	/**
	 * 根据字码和类型获取抽样方案
	 * @param codeLetter
	...
	private boolean existsSampleScheme(SampleScheme sampleScheme){
		...
		if(sampleScheme.getBaseType()!= null){
			hql += " and s.countType = ?";
