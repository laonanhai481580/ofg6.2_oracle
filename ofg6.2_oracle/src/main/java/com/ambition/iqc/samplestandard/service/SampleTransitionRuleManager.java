...
public class SampleTransitionRuleManager {
	 ...
	 */
	public void saveSampleTransitionRule(JSONArray sampleTranstionArray,String baseType,String usebaseType) throws Exception{
		//删除原来的设置
		...
		for(int i=0;i<sampleTranstionArray.size();i++){
			...
			sampleTransitionRule.setBaseType(baseType);
		...
		if(baseType != null){
			...
			useBaseType.setBaseType(usebaseType);
