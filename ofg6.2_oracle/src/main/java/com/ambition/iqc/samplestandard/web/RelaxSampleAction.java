...
public class RelaxSampleAction extends CrudActionSupport<SampleScheme> {
	...
	public String getTpListDatas() throws Exception {
		try {
			page = sampleSchemeManager.getListDatas(page,SampleCodeLetter.ANSI_TYPE,SampleScheme.RELAX_TYPE);
			this.renderText(PageUtils.dynamicPageToJson(page,new DynamicColumnValues(){
				public void addValuesTo(List<Map<String, Object>> result) {
					for(Map<String, Object> map : result){
						Long id = Long.valueOf(map.get("id").toString());
	...
	public String getMilListDatas() throws Exception {
		try {
			page = sampleSchemeManager.getListDatas(page,SampleCodeLetter.MIL105E_TYPE,SampleScheme.RELAX_TYPE);
			this.renderText(PageUtils.dynamicPageToJson(page,new DynamicColumnValues(){
				public void addValuesTo(List<Map<String, Object>> result) {
					for(Map<String, Object> map : result){
						Long id = Long.valueOf(map.get("id").toString());
