...
public class SampleCodeLetterAction extends CrudActionSupport<SampleCodeLetter> {
	...
	protected void prepareModel() throws Exception {
		if(id == null){
			...
			sampleCodeLetter.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
		}else{
	...
	public String getTpListDatas() throws Exception {
		try {
			page = sampleCodeLetterManager.getListDatas(page,SampleCodeLetter.ANSI_TYPE);
			renderText(PageUtils.pageToJson(page));
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	//样本代字对照表
	@Action("mil-list")
	public String getMilList() throws Exception {
		return SUCCESS;
	}
	...
	public String getMilListDatas() throws Exception {
		try {
			page = sampleCodeLetterManager.getListDatas(page,SampleCodeLetter.MIL105E_TYPE);
			renderText(PageUtils.pageToJson(page));
		} catch (Exception e) {
			e.printStackTrace();
		}
