...
public class SampleTransitionRuleAction extends CrudActionSupport<SampleTransitionRule> {
	...
	public String save() throws Exception {
		try {
			...
			String useBaseType = Struts2Utils.getParameter("useBaseType");
			JSONArray jsonArray = JSONArray.fromObject(Struts2Utils.getParameter("transtionRuleStrs"));
			sampleTransitionRuleManager.saveSampleTransitionRule(jsonArray,baseType,useBaseType);
			createMessage("保存成功!");
		} catch (Exception e) {
	...
	public String input() throws Exception {
		...
		for(SampleTransitionRule transitionRule : sampleTransitionRules){
			...
			baseType=transitionRule.getBaseType();
		}
		...
		Struts2Utils.getRequest().setAttribute("baseType", baseType);
		ActionContext.getContext().put("baseTypes",ApiFactory.getSettingService().getOptionsByGroupCode("iqc-sampling-base-type"));
		return SUCCESS;
	}
