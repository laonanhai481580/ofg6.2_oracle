...
public class InspectionFromOverdueEmailAction extends CrudActionSupport<InspectionFromOverdueEmail>{
	...
	protected void prepareModel() throws Exception {
		if(id==null){
			...
			inspectionFromOverdueEmail.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
		}else{
	...
	public String save() throws Exception {
		try {
			if(id == null){
				...
				logUtilDao.debugLog("保存", inspectionFromOverdueEmail.toString());
			}else{
				inspectionFromOverdueEmail.setLastModifiedTime(new Date());
				inspectionFromOverdueEmail.setLastModifier(ContextUtils.getUserName());
				overdueEmailManager.saveInspectionFromOverdueEmail(inspectionFromOverdueEmail);
				logUtilDao.debugLog("修改", inspectionFromOverdueEmail.toString());
			}
	...
	public String export() throws Exception {
		try {
			Page<InspectionFromOverdueEmail> page = new Page<InspectionFromOverdueEmail>(65545);
			page=overdueEmailManager.getInspectionFromOverdueEmailPage(page);
	...
	@Action("save-old")
	@LogInfo(optType="同步",message="同步基础维护物料")
	public String saveOld() throws Exception {
		JSONObject result = new JSONObject();
		List<InspectingIndicator> list=inspectingIndicatorManager.getAllInspectingIndicators();
		for(InspectingIndicator inspectingIndicator : list){			
			String materielCode=inspectingIndicator.getMaterielCode();
			if(!overdueEmailManager.isExistOverdue(null, materielCode, inspectingIndicator.getBusinessUnitName())){
				InspectionFromOverdueEmail inspectionFromOverdueEmail=new InspectionFromOverdueEmail();
				inspectionFromOverdueEmail.setCreatedTime(new Date());
				inspectionFromOverdueEmail.setCompanyId(ContextUtils.getCompanyId());
				inspectionFromOverdueEmail.setCreator(ContextUtils.getUserName());
				inspectionFromOverdueEmail.setLastModifiedTime(new Date());
				inspectionFromOverdueEmail.setLastModifier(ContextUtils.getUserName());
				inspectionFromOverdueEmail.setBusinessUnitName(inspectingIndicator.getBusinessUnitName());
				inspectionFromOverdueEmail.setMaterielCode(materielCode);
				overdueEmailManager.saveInspectionFromOverdueEmail(inspectionFromOverdueEmail);
			}		
		}
		try{			
