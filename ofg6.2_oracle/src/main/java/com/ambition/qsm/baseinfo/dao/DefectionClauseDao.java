...
public class DefectionClauseDao extends HibernateDao<DefectionClause,Long>{
	public Page<DefectionClause> list(Page<DefectionClause> page){
		return findPage(page, "from DefectionClause d where d.companyId=?",ContextUtils.getCompanyId());
	}
	
	public List<DefectionClause> getAllDefectionClause(){
		return find("from DefectionClause d where d.companyId=?",ContextUtils.getCompanyId());
	}

    public Page<DefectionClause> search(Page<DefectionClause> page) {
        return searchPageByHql(page, "from DefectionClause d where d.companyId=?",ContextUtils.getCompanyId());
    }
