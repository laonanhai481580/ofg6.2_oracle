...
public class SystemCertificateDao extends HibernateDao<SystemCertificate,Long>{
	public Page<SystemCertificate> list(Page<SystemCertificate> page){
		return findPage(page, "from SystemCertificate d where d.companyId=?",ContextUtils.getCompanyId());
	}
	
	public List<SystemCertificate> getAllSystemCertificate(){
		return find("from SystemCertificate d where d.companyId=?",ContextUtils.getCompanyId());
	}

    public Page<SystemCertificate> search(Page<SystemCertificate> page) {
        return searchPageByHql(page, "from SystemCertificate d where d.companyId=?",ContextUtils.getCompanyId());
    }
}
