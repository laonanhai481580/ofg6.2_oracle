...
public class SystemMaintenanceDao extends HibernateDao<SystemMaintenance,Long>{
	public Page<SystemMaintenance> list(Page<SystemMaintenance> page){
		return findPage(page, "from SystemMaintenance d where d.companyId=?",ContextUtils.getCompanyId());
	}
	
	public List<SystemMaintenance> getAllSystemMaintenance(){
		return find("from SystemMaintenance d where d.companyId=?",ContextUtils.getCompanyId());
	}

    public Page<SystemMaintenance> search(Page<SystemMaintenance> page) {
        return searchPageByHql(page, "from SystemMaintenance d where d.companyId=?",ContextUtils.getCompanyId());
    }
}
