...
public class SystemCertificateAction extends BaseAction<SystemCertificate>{
	...
	protected void prepareModel() throws Exception {
		if(id==null){
			...
			systemCertificate.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
			User user= ApiFactory.getAcsService().getUserByLoginName(ContextUtils.getLoginName());
			String subName=user.getSubCompanyName();
			systemCertificate.setFactoryClassify(subName);
		}else{
...
//		String processSection = Struts2Utils.getParameter("processSection");
		...
		try{
			...
			if("TP".equals(companyName)){
				if(weight==3){
					page = systemCertificateManager.listState(page, type,null);
				}else{
					page = systemCertificateManager.listState(page, type,subName);
				}
			}else{
				page = systemCertificateManager.listState(page, type,null);
			}
	...
	public String export() throws Exception {
		try {
			...
			User user= ApiFactory.getAcsService().getUserByLoginName(ContextUtils.getLoginName());
			String subName=user.getSubCompanyName();
			page = systemCertificateManager.listState(page, "N",subName);
