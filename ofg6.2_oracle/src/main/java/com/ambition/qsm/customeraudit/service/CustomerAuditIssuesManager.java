...
public class CustomerAuditIssuesManager extends AmbWorkflowManagerBase<CustomerAuditIssues>{
	...
	public String deleteEntity(String ids) {
		...
		for (String id : deleteIds) {
			CustomerAuditIssues report=getEntity(Long.valueOf(id));
			deleteEntity(report);
	...
	public String importDatas(File file) throws Exception{
		...
		while(rows.hasNext()){
			...
			try {
				...
				customerAuditIssues.setFactoryClassify(subName);
				customerAuditIssues.setHiddenState("N");
				customerAuditIssues.setConsignableDate(new Date());
				for(String key : objMap.keySet()){
					CommonUtil.setProperty(customerAuditIssues,key, objMap.get(key));
				}
	...
	public Page<CustomerAuditIssues> listState(Page<CustomerAuditIssues> page,String state ,String str){
		String hql = " from CustomerAuditIssues e where e.hiddenState=? ";
		...
		return customerAuditIssuesDao.searchPageByHql(page, hql, searchParams.toArray());
	}
}
