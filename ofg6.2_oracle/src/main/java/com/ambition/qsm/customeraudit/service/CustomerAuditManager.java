...
public class CustomerAuditManager {
	...
	public String importDatas(File file) throws Exception{
		...
		while(rows.hasNext()){
			...
			try {
				...
				customerAudit.setHiddenState("N");
				User user= ApiFactory.getAcsService().getUserByLoginName(ContextUtils.getLoginName());
				String subName=user.getSubCompanyName();
				customerAudit.setFactoryClassify(subName);
				for(String key : objMap.keySet()){
					CommonUtil.setProperty(customerAudit,key, objMap.get(key));
				}
				customerAuditDao.save(customerAudit);
	...
	public Page<CustomerAudit> listState(Page<CustomerAudit> page,String state ,String str){
		String hql = " from CustomerAudit e where e.hiddenState=? ";
		...
		return customerAuditDao.searchPageByHql(page, hql, searchParams.toArray());
	}
}
