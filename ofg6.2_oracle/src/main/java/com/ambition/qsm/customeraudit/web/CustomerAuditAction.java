...
public class CustomerAuditAction extends BaseAction<CustomerAudit>{
	...
	protected void prepareModel() throws Exception {
		if(id==null){
			...
			customerAudit.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
...
//		String processSection = Struts2Utils.getParameter("processSection");
		...
		try{
			...
			if("TP".equals(companyName)){
				if(weight==3){
					page = customerAuditManager.listState(page, type,null);
				}else{
					page = customerAuditManager.listState(page, type,subName);
				}
			}else{
				page = customerAuditManager.listState(page, type,null);
			}
