...
public class AuditorLibraryDao extends HibernateDao<AuditorLibrary,Long>{
	public Page<AuditorLibrary> list(Page<AuditorLibrary> page){
		return findPage(page, "from AuditorLibrary d where d.companyId=?",ContextUtils.getCompanyId());
	}
	
	public List<AuditorLibrary> getAllAuditorLibrary(){
		return find("from AuditorLibrary d where d.companyId=?",ContextUtils.getCompanyId());
	}

    public Page<AuditorLibrary> search(Page<AuditorLibrary> page) {
        return searchPageByHql(page, "from AuditorLibrary d where d.companyId=?",ContextUtils.getCompanyId());
    }
}
