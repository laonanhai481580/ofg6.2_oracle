...
public class AuditorLibraryManager {
	...
	public String importDatas(File file) throws Exception{
		...
		while(rows.hasNext()){
			...
			try {
				...
				auditorLibrary.setHiddenState("N");
				User user= ApiFactory.getAcsService().getUserByLoginName(ContextUtils.getLoginName());
				String subName=user.getSubCompanyName();
				auditorLibrary.setFactoryClassify(subName);
				for(String key : objMap.keySet()){
					CommonUtil.setProperty(auditorLibrary,key, objMap.get(key));
				}
				auditorLibraryDao.save(auditorLibrary);
	...
	public Page<AuditorLibrary> listState(Page<AuditorLibrary> page,String state ,String str){
		String hql = " from AuditorLibrary e where e.hiddenState=? and e.companyId=?";
		...
		return auditorLibraryDao.searchPageByHql(page, hql, searchParams.toArray());
	}
}
