...
public class ProblemsManager extends AmbWorkflowManagerBase<Problems>{
		...
		while(rows.hasNext()){
			...
			try {
				...
				for(String key : objMap.keySet()){
					CommonUtil.setProperty(problems,key, objMap.get(key));
				}
	...
	public Page<Problems> listState(Page<Problems> page,String state ,String str){
		String hql = " from Problems e where e.hiddenState=? and e.companyId=?";
		...
		return problemsDao.searchPageByHql(page, hql, searchParams.toArray());
	}
	/**
	 * 删除实体，流程相关文件都删除
	 * @param ids
	...
	public String deleteEntity(String ids) {
		...
		for (String id : deleteIds) {
			Problems report=getEntity(Long.valueOf(id));
			deleteEntity(report);
