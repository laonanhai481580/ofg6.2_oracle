...
public class AuditorLibraryAction extends BaseAction<AuditorLibrary>{
	...
	protected void prepareModel() throws Exception {
		if(id==null){
			...
			auditorLibrary.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
...
//		String processSection = Struts2Utils.getParameter("processSection");
		...
		try{
			...
			if("TP".equals(companyName)){
				if(weight==3){
					page = auditorLibraryManager.listState(page, type,null);
				}else{
					page = auditorLibraryManager.listState(page, type,subName);
				}
			}else{
				page = auditorLibraryManager.listState(page, type,null);
			}
