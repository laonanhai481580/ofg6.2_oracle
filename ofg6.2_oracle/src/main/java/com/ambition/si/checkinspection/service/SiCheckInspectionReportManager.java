...
public class SiCheckInspectionReportManager {
	...
	public String deleteSiCheckInspectionReport(String deleteIds) {
		String message = "";
        Integer deleteNum = 0;
		...
		for(String id :ids){
			if(id != null){
				...
				if(siCheckInspectionReport.getCreator()!=null&&siCheckInspectionReport.getCreator().equals(ContextUtils.getLoginName())){
					...
					sb.append(siCheckInspectionReport.getFormNo() + ",");
					deleteNum++;
				}else{
	        		dontDeleteNum++;
	        	}
	...
	public Page<SiCheckInspectionReport> listHide(Page<SiCheckInspectionReport> page){
		return siCheckInspectionReportDao.listHide(page);
	}
	...
	public String hiddenState(String hideId,String type){
		...
		for(String id : ids){
			SiCheckInspectionReport report = siCheckInspectionReportDao.get(Long.valueOf(id));
			if("Y".equals(type)){
				report.setHiddenState("N");
			}else{
				report.setHiddenState("Y");
			}
			siCheckInspectionReportDao.save(report);
			...
			for (int i = 0; i < childrenInfo.size(); i++) {
				...
				for(Object key : js.keySet()){
					if(key != null && js.get(key) != null){
						try{
							CommonUtil.setProperty(tr, key.toString(), js.get(key));
						}catch (Exception e) {
							e.printStackTrace();
						}
					}
