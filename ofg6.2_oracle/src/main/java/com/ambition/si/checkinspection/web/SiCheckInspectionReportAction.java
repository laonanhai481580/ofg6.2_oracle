...
public class SiCheckInspectionReportAction extends CrudActionSupport<SiCheckInspectionReport> {
	...
	protected void prepareModel() throws Exception {
		if(id==null){			
			...
			siCheckInspectionReport.setBusinessUnitCode(CommonUtil.getBusinessUnitCode(ContextUtils.getCompanyName()));
		}else {			
	...
	public String getListDatasHide() throws Exception {
		try {
			page = siCheckInspectionReportManager.listHide(page);
		} catch (Exception e) {
			e.printStackTrace();
		}
		this.renderText(PageUtils.pageToJson(page));
	...
	public String delete() throws Exception {
		try{
			String message=siCheckInspectionReportManager.deleteSiCheckInspectionReport(deleteIds);
			String str1=message.split("~")[0];
			String str2=message.split("~")[1];
			renderText(str2);
			Struts2Utils.getRequest().setAttribute(LogInfo.MESSAGE_ATTRIBUTE,"删除SI检验报告，编号为:"+str1);
		}catch (Exception e) {
			log.error("删除失败!",e);
   ...
   public void exportReport() throws Exception {
	...
	public String hiddenState(){
		...
		String formNo="";
		try {
			formNo=siCheckInspectionReportManager.hiddenState(eid,type);
