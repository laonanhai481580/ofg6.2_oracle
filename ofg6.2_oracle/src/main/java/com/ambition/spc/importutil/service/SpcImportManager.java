...
//	private long executeJiangge = 1*60*1000;
	...
	public void executeImport(Session session,Long featureId,final List<JSONObject> valueMaps){
		...
		List<?> list2 = session.createSQLQuery(sql).setParameter(0,qualityFeature.getTargetTableName()).list();
		if(qualityFeature.getTargetTableName()==null||list2.size()==0){
			return;
		}
		...
		for(FeatureLayer layer : qualityFeature.getFeatureLayers()){
			...
			String columnName = layer.getDetailCode().toUpperCase();
			if(!existColumnMap.containsKey(columnName)){
				session.createSQLQuery("alter table "+qualityFeature.getTargetTableName()+" add "+columnName+" VARCHAR2(255)").executeUpdate();
			}
