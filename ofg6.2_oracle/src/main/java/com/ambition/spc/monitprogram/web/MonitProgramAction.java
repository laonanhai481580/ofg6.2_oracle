		...
		for(MonitPoint monitPoint : monitPoints){
			...
			if(iscpk.equals("Y")){
				...
				for(int i=0;i<monitPoint.getMonitQualityFeature().size();i++){
					...
					/*if(monitQualityFeature.getQualityFeatureId()==28123667){
						System.out.println(monitQualityFeature.getQualityFeatureId());
					}*/
					if(qualityFeature!=null){
					JLcalculator jLcalculator=new JLcalculator();
					JLOriginalData originalData=new JLOriginalData();
					//封装originalDate
					jlanalyseDrawManager.calculatJl(jLcalculator, String.valueOf(qualityFeature.getId()),startDate,endDate,originalData,"analysis","",new JSONArray(),null);
