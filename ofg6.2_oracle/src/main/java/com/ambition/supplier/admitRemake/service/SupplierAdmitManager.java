...
public class SupplierAdmitManager extends AmbWorkflowManagerBase<SupplierAdmit>{
	...
	public String deleteEntity(String ids) {
		...
		for (String id : deleteIds) {
			...
			if(report.getGpMaterialId()!=null&&!"".equals(report.getGpMaterialId())){
				deleteGpMaterialRelevance(report);
			}
			deleteEntity(report);
	...
	public void deleteEntity(SupplierAdmit entity){
		...
		if(!companyName.equals("")&&companyName!=null){
			String sql = "insert into SUPPLIER_DELETE_MARK (id,source_id,source_unit,source_table) values("+entity.getId()+","+entity.getId()+",'"+companyName+"','"+SupplierDeleteMark.SUPPLIER_SUPPLIER_ADMIT_ALL+"')";
	...
	public void saveUser(SupplierAdmit report, Department department) {
		...
		List<?> users = supplierAdmitDao.getSession().createQuery(hql).setParameter(0, report.getSupplierCode().trim())
				.list();
		User user = new User();
		boolean isNew = false;
		if (users.size() <= 0) {
	...
	public Page<SupplierAdmit> listState(Page<SupplierAdmit> page,String code,String state){
		String caName= Struts2Utils.getParameter("caName");
		...
		if(caName!=null){
			caName="已完结";
			hql=hql+" and e.taskProgress = ?";
			searchParams.add(caName);
		}else{
			caName="已完结";
			hql=hql+" and ( e.taskProgress <> ? or e.taskProgress is null )";
			searchParams.add(caName);
		}
		return supplierAdmitDao.searchPageByHql(page, hql, searchParams.toArray());
	}
	public Page<SupplierAdmit> listInventory(Page<SupplierAdmit> page,String state){
		String hql = " from SupplierAdmit e where  e.companyId=?";
		List<Object> searchParams = new ArrayList<Object>();
		searchParams.add(ContextUtils.getCompanyId());
		if(state!=null ){
	...
	public Page<SupplierAdmit> listLost(Page<SupplierAdmit> page){
		String hql = " from SupplierAdmit e where  e.companyId=? and e.loseTime is not null and e.loseTime<=? ";
		List<Object> searchParams = new ArrayList<Object>();
		searchParams.add(ContextUtils.getCompanyId());
		searchParams.add(new Date());
		return supplierAdmitDao.searchPageByHql(page, hql, searchParams.toArray());
	}
	...
	public SupplierAdmit getSupplierAdmit(String supplierCode,String materialCode){
		...
		List<SupplierAdmit> supplierAdmits = supplierAdmitDao.find(hql,searchParams.toArray());
	...
	public SupplierAdmit getSupplierAdmit(String supplierCode,String materialCode,Long id){
		String hql = "from SupplierAdmit s where s.companyId=? and s.materialCode = ? and s.supplierCode=? ";
		List<Object> searchParams = new ArrayList<Object>();
		searchParams.add(ContextUtils.getCompanyId());
		searchParams.add(materialCode);
...
//				supplierAdmit.setFactoryClassify(subName);
	...
	public SupplierAdmit findSupplierMaterial(GpMaterial gpMaterial){
		String hql="from SupplierAdmit s where s.companyId=? and s.supplierCode = ? and s.materialCode = ? ";
		List<SupplierAdmit> supplierAdmits=supplierAdmitDao.find(hql, ContextUtils.getCompanyId(),gpMaterial.getSupplierCode(),gpMaterial.getMaterialCode().trim());
		SupplierAdmit report=null;
		if(supplierAdmits.size()>0){
			report=supplierAdmits.get(0);
			...
			supplierAdmitDao.save(report);
		}
		return report;
	}
	public void deleteGpMaterialRelevance(SupplierAdmit entity){
		GpMaterial gpMaterial=gpMaterialDao.get(Long.valueOf(entity.getGpMaterialId()));
		if(gpMaterial!=null){
			gpMaterial.setApprovalId("");
			gpMaterial.setApprovalNo("");
