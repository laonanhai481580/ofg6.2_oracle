...
public class SupplierApprovalManager extends AmbWorkflowManagerBase<SupplierApproval>{
	...
	public void saveUser(SupplierApproval report, Department department) {
		...
		List<?> users = supplierApprovalDao.getSession().createQuery(hql).setParameter(0, report.getSupplierCode().trim())
				.list();
		User user = new User();
		boolean isNew = false;
		if (users.size() <= 0) {
	...
	public Page<SupplierApproval> listState(Page<SupplierApproval> page,String code){
		...
		if(caName!=null){
			caName="流程结束";
			hql=hql+" and e.workflowInfo.currentActivityName = ?";
			searchParams.add(caName);
		}else{
			caName="流程结束";
			hql=hql+" and (e.workflowInfo.currentActivityName <> ? or e.workflowInfo.currentActivityName is null) ";
			searchParams.add(caName);
		}
	...
	public SupplierApproval findSupplierMaterial(GpMaterial gpMaterial){
		String hql="from SupplierApproval s where s.companyId=? and s.supplierCode = ? and s.materialCode = ? ";
		List <SupplierApproval> supplierApprovals=supplierApprovalDao.find(hql, ContextUtils.getCompanyId(),gpMaterial.getSupplierCode(),gpMaterial.getMaterialCode().trim());
		SupplierApproval report=null;
		if(supplierApprovals.size()>0){
			report=supplierApprovals.get(0);
			...
			supplierApprovalDao.save(report);
		}
		return report;
	}
	public SupplierApproval getSupplierApproval(String SupplierCode,String MaterialCode,Long id){
		String hql="from SupplierApproval s where s.companyId=? and s.supplierCode = ? and s.materialCode = ? ";
		List<Object> searchParams = new ArrayList<Object>();
		searchParams.add(ContextUtils.getCompanyId());
		...
		List <SupplierApproval> supplierApprovals=supplierApprovalDao.find(hql,searchParams.toArray());
		if(supplierApprovals.isEmpty()){
			return null;
		}else{
			return supplierApprovals.get(0);
		}
	}
	public void deleteGpMaterialRelevance(SupplierApproval entity){
