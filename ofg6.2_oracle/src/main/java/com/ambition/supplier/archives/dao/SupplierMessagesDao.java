...
public class SupplierMessagesDao extends  HibernateDao<SupplierMessages,Long>{
	...
	public List<SupplierMessages> searchByFileName(String fileName, Long id) {
		if(null!=id&&!"".equals(id)){
        	return find("from SupplierMessages t where t.fileName=? and t.id != ? and t.companyId=? and t.isDelete=false ",fileName,id,ContextUtils.getCompanyId());
    	}
    	return find("from SupplierMessages t where t.fileName=? and t.companyId=? and t.isDelete=false ",fileName,ContextUtils.getCompanyId());
	}
