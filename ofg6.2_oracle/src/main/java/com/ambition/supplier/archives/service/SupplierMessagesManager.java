...
public class SupplierMessagesManager {
	...
	public Page<SupplierMessages> searchByPage(Page<SupplierMessages> page) {
		return supplierMessagesDao.searchPageByHql(page, "from SupplierMessages t where t.isNewVersion=? and t.isDelete=? order by t.id desc",true,false);
	}
	...
	public void deleteSupplierMessages(String deleteIds) {
		...
		for(String id : ids){
			...
			if(supplierMessages != null){
				supplierMessages.setIsDelete(true);		
				supplierMessages.setDeleteDate(new Date());
				supplierMessages.setDeleteMan(ContextUtils.getUserName());
				supplierMessages.setDeleteManLogin(ContextUtils.getLoginName());
