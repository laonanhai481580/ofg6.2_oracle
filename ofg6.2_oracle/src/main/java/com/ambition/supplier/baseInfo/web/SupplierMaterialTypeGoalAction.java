		...
		try {
			...
			if("check".equals(type)){
				if("欧菲科技-FPM".equals(ContextUtils.getCompanyName())){
					result.put("value", supplierMaterialTypeGoal.getComingUpperLimit()/100);
				}else{
					result.put("value", supplierMaterialTypeGoal.getComingLowerLimit()/100);
					result.put("L", supplierMaterialTypeGoal.getComingLowerLimit()/100);
					result.put("U", supplierMaterialTypeGoal.getComingUpperLimit()/100);
				}
			}else{
				result.put("value", supplierMaterialTypeGoal.getLineUpperLimit()/100);
				result.put("L", supplierMaterialTypeGoal.getLineLowerLimit()/100);
				result.put("U", supplierMaterialTypeGoal.getLineUpperLimit()/100);
			}
