...
//				}
	...
	public EvaluatingIndicator findByName(String name) {
		// TODO Auto-generated method stub
		String hql = " from EvaluatingIndicator e where e.companyId = ? and e.name = ?";
		List<EvaluatingIndicator> cator = evaluatingIndicatorDao.find(hql,ContextUtils.getCompanyId(),name);
		if(cator.size()==0){
			return null;
		}else{
			return cator.get(0);
	...
	public List<EvaluatingIndicator> findByParentId(Long id) {
		// TODO Auto-generated method stub
		String eHql = " from EvaluatingIndicator e where e.parent.id=? order by e.orderByNum asc ";
		List<EvaluatingIndicator> lists = evaluatingIndicatorDao.find(eHql,id);
		return lists;
	}
