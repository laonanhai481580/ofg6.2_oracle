...
public class EvaluateManager {
	 ...
	 * @return
	 */
	public Page<Evaluate> search(Page<Evaluate> page, Long supplierId, Long modelId) {
		return evaluateDao.search(page, supplierId,modelId);
	}
	 ...
	 * @return
	 */
	public Evaluate findEvaluateBySupplerAndYearMonthAndCycle(Supplier supplier ,
			Integer year, Integer month, Long estimateModelId,String materialType,String cycle) {
		return evaluateDao.findEvaluateBySupplerAndYearMonthAndCycle(supplier, year,
...
//			}
		...
		String modelIndicatorIds[] = params.getString("modelIndicatorIds")
				.split(",");
//		EvaluatingIndicator evaluatingIndicator = evaluatingIndicatorManager.findByName("RoHS检测不合格");
		String roShId = null;
//		if(evaluatingIndicator!=null){
//			roShId = evaluatingIndicator.getId();
//		}
		...
		for (String modelIndicatorId : modelIndicatorIds) {
			if (StringUtils.isNotEmpty(modelIndicatorId)) {
				String  bad = params.getString("name_"+modelIndicatorId);
				if("RoHS检测不合格".equals(bad)){
					roShId=modelIndicatorId;
				}
...
//				evaluateDetail.setSqeDepartmentName(evaluate.getSqeDepartmentName());
				...
				for (Object key : params.keySet()) {
					...
					if (key.toString().endsWith("_" + modelIndicatorId)) {
						...
						if(key.equals("score_"+roShId)&&roShId.equals(modelIndicatorId)){
							String keyValue = StringUtils.isEmpty(params.get(key).toString())?"0.0":params.get(key).toString();
							Double roshValue = Double.valueOf(keyValue);
							if(roshValue>=1){
	...
	private void calculateRealPointsBySupplier(Supplier supplier,
			Long estimateModelId, Evaluate evaluate,Boolean roshD) {
