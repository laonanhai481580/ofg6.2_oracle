...
//        }
	...
	public void rendHistoryDatas() {
		...
		session.createSQLQuery(sqlDel)
			.executeUpdate();
		String sql = "select t.supplier_Id||t.supplier_Name||t.material_Type||t.evaluate_year||t.evaluate_Month, t.supplier_Id,t.SUPPLIER_NAME,t.material_Type,t.evaluate_year,t.evaluate_Month,t.estimate_model_name,t.real_total_points,t.is_Submit,t.is_Rohs,t.cycle,t.model_Id,t.factory_Classify from SUPPLIER_EVALUATE t";
		...
		for(String key : map.keySet()){
			...
			for(int i=0;i<objArray.size();i++){
				...
				String isSubmit=objs[8]==null?"":objs[8].toString();
				if(objs[9]!=null){
					isGrade=objs[9].toString();
				}
				if(i==0){
					...
					totalView.setCycle(objs[10].toString());
					totalView.setModelId(objs[11]==null?"":objs[11].toString());
					totalView.setFactoryClassify(objs[12]==null?"":objs[12].toString());
				}
	...
	public String selectIqcCount(String id){
		Supplier supplier=supplierDao.get(Long.valueOf(id));
		String code=supplier.getCode();
		String sql = "select count(*) from IQC_IIAR s where s.SUPPLIER_CODE =?";
		Query query = supplierEvaluateTotalViewDao.getSession().createSQLQuery(sql).setParameter(0, code);
		List<?> list = query.list();
//		Integer mount = Integer.valueOf(list.get(0).toString());
		return list.get(0).toString();
	}
	public Page<SupplierEvaluateTotalView> listState(Page<SupplierEvaluateTotalView> page,String state){
		String hql = " from SupplierEvaluateTotalView e where e.evaluateType=?";
		return supplierEvaluateTotalViewDao.searchPageByHql(page, hql, state);
	}
}
