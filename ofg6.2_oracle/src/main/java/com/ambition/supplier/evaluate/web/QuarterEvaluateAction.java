...
//	}
	...
	public String add() throws Exception {
		...
		if(supplier==null){
			addActionMessage("请先检查供应商名录!");
			return SUCCESS;
		}
		Long estimateModelIdInner = supplier.getEstimateModelId();
		String modelId = Struts2Utils.getParameter("modelId");
		if (StringUtils.isNotEmpty(modelId)) {
			estimateModelIdInner = Long.valueOf(modelId);
		}
		...
		List<EstimateModel> models = estimateModelManager.getAllParentModel();
		List<Option> modelOptions = new ArrayList<Option>();
		for (EstimateModel model : models) {
			Option option = new Option();
			option.setName(model.getName());
			option.setValue(model.getId() + "");
			if (estimateModelIdInner == null) {
				estimateModelIdInner = model.getId();
			}
			modelOptions.add(option);
		}
		ActionContext.getContext().put("estimateModelOptions", modelOptions);
		...
		if (evaluate != null) {
			...
			if (supplier == null) {
				...
				addActionMessage("请先选择供应商!");
				return SUCCESS;
			} else {
				ActionContext.getContext().put("supplierName", supplier.getName());
				ActionContext.getContext().put("supplierCode", supplier.getCode());
				ActionContext.getContext().put("sqeDepartmentName", supplier.getShareGroup());
				supplierId = supplier.getId();
				EstimateModel estimateModel = estimateModelManager.getEstimateModelById(estimateModelIdInner);
					...
					if (estimateModelId == null) {
						...
						if (estimateModel == null) {
							...
							for (int i = 0; i < 12 / interval; i++) {
								...
								Option option = new Option();
								//"一个月".equals(estimateModel.getCycle())
								if("一个月".equals(estimateModel.getCycle())){
									if(tempMonth==1){
										option.setName("一月");
									}else if(tempMonth==2){
										option.setName("二月");
							...
							evaluate = evaluateManager.findEvaluateBySupplerAndYearMonthAndCycle(supplier,evaluateYear, evaluateMonth, estimateModelId,materialType,estimateModel.getCycle());
							if (evaluate == null) {
								evaluate = new Evaluate();
								evaluate.setSupplierId(supplier.getId());
								evaluate.setSupplierName(supplier.getName());
								evaluate.setMaterialType(materialType);
								evaluate.setSqeDepartmentName(supplier.getShareGroup());
							...
							ActionContext.getContext().put("estimateModelIdInner", estimateModelIdInner);
//							ActionContext.getContext().put("estimateModelId", estimateModelId);
							if(modelId==null){
								if(evaluate.getModelId()!=null){
									modelId=evaluate.getModelId().toString();
									ActionContext.getContext().put("estimateModelIdInner", modelId);
								}
							}
							ActionContext.getContext().put("modelId", modelId);
...
//				||modelIndicator.getTotalPoints() <= 0
		...
		for(EvaluatingIndicator evaluatingIndicator : topEvaluatingIndicators){
			...
			if(modelIndicator != null){
				...
				List<EvaluatingIndicator> indicators = new ArrayList<EvaluatingIndicator>();
				indicators = evaluatingIndicatorManager.findByParentId(evaluatingIndicator.getId());
				for(EvaluatingIndicator level2 : indicators){
...
//				}
	...
	public String getLedger(){
		...
		if(supplier != null){
			...
			Long estimateModelIdInner = supplier.getEstimateModelId();
			String modelId = Struts2Utils.getParameter("modelId");
			if (StringUtils.isNotEmpty(modelId)&&!modelId.equals("undefined")) {
				estimateModelIdInner = Long.valueOf(modelId);
				estimateModel = estimateModelManager.getEstimateModelById(estimateModelIdInner);
			}
			List<EstimateModel> models = estimateModelManager.getAllParentModel();
			List<Option> modelOptions = new ArrayList<Option>();
			for (EstimateModel model : models) {
...
//				}
	...
	public String getEvaluates() throws Exception {
		try{
			String modelId = Struts2Utils.getParameter("modelId");
//			estimateModel=estimateModelManager.getEstimateModelById(Long.valueOf(modelId));
			page = evaluateManager.search(page,supplierId,Long.valueOf(modelId));
		}catch (Exception e) {
			logger.error(e);
	...
	public String exports() throws Exception {
		String modelId = Struts2Utils.getParameter("modelId");
		page = evaluateManager.search(new Page<Evaluate>(Integer.MAX_VALUE),supplierId,Long.valueOf(modelId));
			...
			if(null == indicator.getReadonly() || !indicator.getReadonly()){
				if(evaluateDetail.getName().contains("配合度等级" )||evaluateDetail.getName().contains("配合等级" )){
					if(evaluateDetail.getScore()==null){
						sb.append("<td><select onchange=\"scortChangeObj("+evaluateDetail.getModelIndicatorId()+",this,false);\" value=\""+(evaluateDetail.getScore()==null?"":CommonUtil.retainNDigits(evaluateDetail.getScore(), 3))+"\" style=\"width:40px;\"  name=\"params.score_"+evaluateDetail.getModelIndicatorId()+"\"><option value=\"\"></option> <option value=\"5.0\">优</option><option value=\"3.0\">中</option><option value=\"0.0\">差</option></select></td>");
					}else if(evaluateDetail.getScore()==5.0){
						sb.append("<td><select onchange=\"scortChangeObj("+evaluateDetail.getModelIndicatorId()+",this,false);\" value=\""+(evaluateDetail.getScore()==null?"":CommonUtil.retainNDigits(evaluateDetail.getScore(), 3))+"\" style=\"width:40px;\"  name=\"params.score_"+evaluateDetail.getModelIndicatorId()+"\"><option value=\"\"></option> <option value=\"5.0\" selected='selected' >优</option><option value=\"3.0\">中</option><option value=\"0.0\">差</option></select></td>");
					}else if(evaluateDetail.getScore()==3.0){
						sb.append("<td><select onchange=\"scortChangeObj("+evaluateDetail.getModelIndicatorId()+",this,false);\" value=\""+(evaluateDetail.getScore()==null?"":CommonUtil.retainNDigits(evaluateDetail.getScore(), 3))+"\" style=\"width:40px;\"  name=\"params.score_"+evaluateDetail.getModelIndicatorId()+"\"><option value=\"\"></option> <option value=\"5.0\" >优</option><option selected='selected' value=\"3.0\">中</option><option value=\"0.0\">差</option></select></td>");
					}else if(evaluateDetail.getScore()==0.0){
						sb.append("<td><select onchange=\"scortChangeObj("+evaluateDetail.getModelIndicatorId()+",this,false);\" value=\""+(evaluateDetail.getScore()==null?"":CommonUtil.retainNDigits(evaluateDetail.getScore(), 3))+"\" style=\"width:40px;\"  name=\"params.score_"+evaluateDetail.getModelIndicatorId()+"\"><option value=\"\"></option> <option value=\"5.0\" >优</option><option value=\"3.0\">中</option><option selected='selected'  value=\"0.0\">差</option></select></td>");
					}
					...
					if(evaluateDetail.getScore()==null){
						...
						sb.append("<td><select onchange=\"scortChangeObj("+evaluateDetail.getModelIndicatorId()+",this,false);\" value=\""+(evaluateDetail.getScore()==null?"":CommonUtil.retainNDigits(evaluateDetail.getScore(), 3))+"\" style=\"width:40px;\"  name=\"params.score_"+evaluateDetail.getModelIndicatorId()+"\"><option value=\"\"></option></option> <option value=\"0.0\" selected='selected'>等级A</option> <option value=\"1.0\">等级B</option><option value=\"2.0\">等级C</option><option value=\"3.0\">等级D</option><option value=\"4.0\">等级E</option><option value=\"5.0\">等级F</option></select></td>");
					}else if(evaluateDetail.getScore()==1.0){
...
//			sb.append("<input type=\"hidden\" readonly=\"readonly\"   name=\"params.orderByNum_"+evaluateDetail.getModelIndicatorId()+"\" value=\""+indicator.getOrderByNum()+"\" />");
	...
	private String createModelTableByEvaluateForTotal(List<EvaluateDetail> evaluateDetails,Map<String,String> scoreMap){
		...
		for(EvaluateDetail evaluateDetail : evaluateDetails){
			...
			totalPoints += evaluateDetail.getTotalPoints()==null?0.0: evaluateDetail.getTotalPoints();
			sb.append("<td>"+CommonUtil.retainNDigits(evaluateDetail.getTotalPoints()==null?0.0: evaluateDetail.getTotalPoints(),0)+"</td>");
