...
//            page = supplierEvaluateTotalViewManager.getViewPage(page);
    ...
    @Action("list-improve")
    public String listImprove() throws Exception {
//    	ActionContext.getContext().put("estimateModelMaps","[]");
    	List<EstimateModel> estimateModel = estimateModelManager.getParentEstimateModel();
		List<Option> estimateModels = new ArrayList<Option>();
		for (int i = 0; i < estimateModel.size(); i++) {
    ...
    @Action("list-improv-datas")
    public String listImprovDatas() throws Exception{
        try {
...
//            page = supplierEvaluateTotalViewManager.getViewPage(page);
            ...
            HttpServletRequest request = (HttpServletRequest) ActionContext.getContext().get(ServletActionContext.HTTP_REQUEST);
        	request.getSession().setAttribute("page",page);
        	supplierEvaluateTotalViewManager.listState(page,state);
            renderText(PageUtils.pageToJson(page));
            return null;
        } catch (Exception e) {
            logUtilDao.debugLog("查询失败", "查询失败"+e.getMessage());
...
//    	 page = supplierEvaluateTotalViewManager.getPage(page);
	...
	public String downloadTemplate() throws Exception {
		try {
			...
			String tableInfo=Struts2Utils.getParameter("tableInfo");
			tableInfo=tableInfo.replaceAll("<td", "<td style='border:#969696 1px solid;border-collapse:collapse;'");
			tableInfo=tableInfo.replaceAll("<th", "<th style='border:#969696 1px solid;border-collapse:collapse;'");
			...
			response.setHeader("Content-Disposition", "attachmen.t; filename="
					+ recommendedName );
    ...
    @Action("iqcCount")
    public String selectIqcCount(){
    	String id = Struts2Utils.getParameter("supplierId");
    	String count=supplierEvaluateTotalViewManager.selectIqcCount(id);
    	renderText(count);
    	return null;
    }
}
